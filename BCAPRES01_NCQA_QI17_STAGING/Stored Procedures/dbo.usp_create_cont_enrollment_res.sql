...
CREATE PROC [dbo].[usp_create_cont_enrollment_res]
...
--
--
--SELECT @lcCurYrStDate ='20070101',
--    @lcCurYrEndDate = '20071231',
--    @liPrevYearFlag = 0,
--    @liPrev2YearFlag = 0,
...
IF @liEligAtEndOfCurYear = 1
    ...
    SELECT DISTINCT a.ihds_member_id
        FROM #dw_hedis_member_elig a
        WHERE @lcCurYrEndDate BETWEEN a.eff_date AND a.term_date
ELSE
...
BEGIN
    ...
    SELECT a.*,
            break_in_coverage_count =	CASE	WHEN prev_day_diff <> 0 and eff_date = @lcCurYrStDate
												THEN 0
												WHEN prev_day_diff <> 0 
												THEN 1 
												WHEN prev_term_date is null and  DATEDIFF(dd,@lcCurYrStDate,eff_date) > 0
												THEN 1 
												ELSE 0 
										END,
            length_of_break_in_cov_in_year = 
							CASE	WHEN prev_term_date is null then DATEDIFF(dd,@lcCurYrStDate,eff_date)
									WHEN prev_day_diff = 0 THEN 0
									WHEN prev_term_date >= @lcCurYrStDate THEN prev_day_diff
                                    ELSE DATEDIFF(dd,@lcCurYrStDate,eff_date)
							END
        INTO #current_year
...
--select * from #current_year where ihds_member_id = '642601'
...
--select * from #current_year_ext1 where ihds_member_id = '4043393'
--select * from #current_year where ihds_member_id = '4043393'
--select * from #res where ihds_member_id = '4043393' 
...
--select * from #prev_year_ext1 where ihds_member_id = '8385'
...
--select * from #res where ihds_member_id = '4043393'  --should not
--select * from #res where ihds_member_id = '4094325' --should be elig
