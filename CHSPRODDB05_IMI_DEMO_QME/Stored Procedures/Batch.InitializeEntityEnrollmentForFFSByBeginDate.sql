...
-- Description:	Peforms the initial data gathering for calculating continuous enrollment for entities based on Begin Date for Fee-For-Service enrollment.
--				(Date Comparer: 0BF46D3A-542D-4C10-BEB7-918080A2B228)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityEnrollmentForFFSByBeginDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '0BF46D3A-542D-4C10-BEB7-918080A2B228') AND
		...
		SELECT	COUNT(CASE WHEN Allow = 1 THEN EntityCritID END) AS CountAllowed,
				...
				COUNT(CASE WHEN Allow = 0 AND ISNULL(QtyMin, 1) = 1 AND QtyMax IS NULL THEN EntityCritID END) AS CountDenied,
				EntityID,
				OptionNbr
...
GO
GRANT VIEW DEFINITION ON  [Batch].[InitializeEntityEnrollmentForFFSByBeginDate] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityEnrollmentForFFSByBeginDate] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityEnrollmentForFFSByBeginDate] TO [Processor]
GO
