...
-- Description:	Applies additional event base criteria by psuedo-parent claim. (v2)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEventBaseTypeC]
(
	@BatchID int
)
AS
BEGIN
	...
	BEGIN TRY;
		...
		SET @LogObjectName = 'ApplyEventBaseTypeC'; 
		...
		BEGIN TRY;
			...
			IF @BatchID IS NULL
				RAISERROR(' - Applying additional claim event-based details failed!  No batch was specified.', 16, 1);
			...
			INTO	#EventKey
			FROM	Proxy.EventKey 
			WHERE	(EventTypeID = @EventTypeC) AND
			...
			--EVENT TYPE C: Matched on DSClaimID-----------------------------------------------------
			IF EXISTS(SELECT TOP (1) 1 AS N FROM Proxy.EventKey WHERE EventTypeID = @EventTypeC)
				BEGIN;
					...
					WHERE	(CountCriteria > 1) AND
							...
							(EventTypeID = @EventTypeC);
							
					CREATE CLUSTERED INDEX IX_#InitEventBase ON #InitEventBase (DSClaimID, EventID, OptionNbr);
			...
			SET @LogDescr = ' - Applying additional claim event-based details for BATCH ' + ISNULL(CAST(@BatchID AS varchar), '?') + ' succeeded.'; 
		...
		BEGIN CATCH;
			...
			SET @LogEndTime = GETDATE();
			SET @LogDescr = ' - Applying additional claim event-based details for BATCH ' + ISNULL(CAST(@BatchID AS varchar), '?') + ' refresh failed!'; --{FAILURE LOG DESCRIPTION HERE}
...
GO
GRANT VIEW DEFINITION ON  [Batch].[ApplyEventBaseTypeC] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeC] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeC] TO [Processor]
GO
