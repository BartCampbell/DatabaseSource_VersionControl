...
-- Description:	Purges all "Log" schema tables for the specified batch.
-- =============================================
CREATE PROCEDURE [Batch].[PurgeLogTables]
(
	@BatchID int,
	@ForcePurge bit = 0
)
...
BEGIN
	...
	BEGIN TRY;			
		...
		IF ISNULL(@AllowFinalizePurgeLog, 0) = 1 OR ISNULL(@ForcePurge, 0) = 1
			...
			WHERE	(T.TABLE_SCHEMA = 'Log') AND
					(T.TABLE_TYPE = 'BASE TABLE') AND
					(
						(CSPID.COLUMN_NAME IS NOT NULL) OR
						(CBAT.COLUMN_NAME IS NOT NULL)
					) AND
					(T.TABLE_NAME NOT IN ('Errors', 'Progress', 'Reports', 'ReportUsage'))
