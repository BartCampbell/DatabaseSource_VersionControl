...
-- =============================================
CREATE PROCEDURE [Batch].[CombineClaims_v5]
(
	@BatchID int
)
WITH RECOMPILE
...
BEGIN
	...
	SET @DebugCustomerMemberID = '39900';
	/***************************************************************************************************/
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			--1a) Calculate bit-wise specialty values for all providers...
			SELECT	ISNULL(SUM(DISTINCT PS.BitValue), 0) AS BitSpecialties,
					PP.DSProviderID
			INTO	#ProviderSpecialties
			FROM	Proxy.Providers AS PP
					INNER JOIN Proxy.ProviderSpecialties AS PPS
							ON PP.BatchID = PPS.BatchID AND
								PP.DataRunID = PPS.DataRunID AND
			...
			WHERE	(PP.BatchID = @BatchID) AND
					(PP.DataRunID = @DataRunID) AND
					(PP.DataSetID = @DataSetID)
			GROUP BY PP.DSProviderID

			CREATE UNIQUE CLUSTERED INDEX IX_#ProviderSpecialties ON #ProviderSpecialties (DSProviderID);

			--1b) Summarize claim line attributes...
			SELECT	CCL.BeginDate,
					SUM(DISTINCT CA.BitValue) AS BitClaimAttribs,
					MIN(PS.BitSpecialties) AS BitSpecialties,
			...
			FROM	Proxy.ClaimLines AS CCL
					...
					LEFT OUTER JOIN #ProviderSpecialties AS PS
							ON CCL.DSProviderID = PS.DSProviderID
								...
								(
									...
									(
										...
										(
											...
											(CLB.CalcBeginDate BETWEEN IC.BeginDate AND IC.EndDate)
										) AND
										(CLB.EndDate IS NULL)
									)
								)
					...
					INNER JOIN #ClaimLinesBase AS CLB
							ON UC.DSMemberID = CLB.DSMemberID AND
								(
									...
									(UC.BitSpecialties & @BitSpecialtySurg > 0)
								) AND
								(
