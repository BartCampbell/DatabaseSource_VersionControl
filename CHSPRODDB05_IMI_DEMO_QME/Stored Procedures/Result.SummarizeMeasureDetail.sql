...
-- Description:	Summarizes the results from the measure detail.
-- =============================================
CREATE PROCEDURE [Result].[SummarizeMeasureDetail]
(
	@DataRunID int,
	@BatchID int = NULL
)
...
BEGIN
	...
	BEGIN TRY;
		...
		SET @LogObjectName = 'SummarizeMeasureDetail'; 
		...
		BEGIN TRY;				
			...
			DELETE FROM Result.MeasureSummary WHERE (DataRunID = @DataRunID);

			IF NOT EXISTS (SELECT TOP 1 1 FROM Result.MeasureSummary)
				TRUNCATE TABLE Result.MeasureSummary;
				
			UPDATE Result.MeasureDetail SET IsNumeratorAdmin = IsNumerator WHERE ResultTypeID = 1 AND IsNumeratorAdmin IS NULL AND DataRunID = @DataRunID;

			INSERT INTO Result.MeasureSummary
					(Age,
					AgeBandID,
					AgeBandSegID,
					ClinCondID,
			...
			SELECT	RMD.Age,
					...
					SUM(CONVERT(bigint, RMD.IsNumeratorAdmin)) AS IsNumeratorFSS,
			...
			ORDER BY DataSetID, DataRunID, ProductLineID, PopulationID, 
					...
					ClinCondID, AgeBandID, AgeBandSegID, Gender, Age;
			...
			SET @LogDescr = 'Summarizing measure detail completed successfully.'; 
		...
		BEGIN CATCH;
			...
			SET @LogEndTime = GETDATE();
			SET @LogDescr = 'Summarizing measure detail failed!';
