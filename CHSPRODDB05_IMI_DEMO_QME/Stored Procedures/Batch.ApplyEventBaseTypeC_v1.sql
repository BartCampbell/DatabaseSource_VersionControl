...
-- Description:	Populates the Temp.EventBase table with claim event-based matching data. (v1)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEventBaseTypeC_v1]
(
	@BatchID int
)
AS
		...
		BEGIN TRY;
			...
			IF EXISTS(SELECT TOP (1) 1 AS N FROM Proxy.EventKey WHERE EventTypeID = @EventTypeC)
				BEGIN;
					...
					CREATE CLUSTERED INDEX IX_#EventBase ON #EventBase (DSClaimID, EventID, OptionNbr) WITH (FILLFACTOR = 100);
					CREATE STATISTICS ST_#EventBase ON #EventBase (DSClaimID, EventID, OptionNbr);
					...
					FROM	Proxy.ClaimCodes AS CCC
							...
							INNER JOIN #EventBase AS VB
									ON TEK.EventID = VB.EventID AND
										TEK.EventTypeID = @EventTypeC AND
										TEK.OptionNbr = VB.OptionNbr AND
										
										/*Unique JOIN criteria per event type */
										CCL.DSClaimID = VB.DSClaimID AND
										CCC.DSClaimID = VB.DSClaimID;	
			END;			
