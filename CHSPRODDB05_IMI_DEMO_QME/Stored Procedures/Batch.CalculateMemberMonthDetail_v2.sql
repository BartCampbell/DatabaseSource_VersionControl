...
-- =============================================
CREATE PROCEDURE [Batch].[CalculateMemberMonthDetail_v2]
(
	@BatchID int,
		...
		BEGIN TRY;
			...
			IF ISNULL(@CalculateMbrMonths, 1) = 1
				BEGIN
					...
					CREATE UNIQUE CLUSTERED INDEX IX_#Months ON #Months (BenefitID, ProductLineID, ID);
					...
					SELECT DISTINCT
							PB.BenefitID, PB.BitValue
					INTO	#ProductBenefitsBitWised
					FROM	Product.Benefits AS PB
					UNION
					SELECT DISTINCT
							PBR.ChildID AS BenefitID, PB.BitValue
					FROM	Product.BenefitRelationships AS PBR
							INNER JOIN Product.Benefits AS PB
									ON PBR.BenefitID = PB.BenefitID
					ORDER BY BenefitID;
					
					CREATE UNIQUE CLUSTERED INDEX IX_#ProductBenefitsBitWised ON #ProductBenefitsBitWised (BenefitID, BitValue);
					...
					SELECT	t.Age, 
							...
							@BatchID AS BatchID, 
							SUM(DISTINCT PB.BitValue) AS BitBenefits,
							...
							t.PopulationID 
					FROM	#Months AS t
							INNER JOIN #ProductBenefitsBitWised AS PB
									ON t.BenefitID = PB.BenefitID
...
GO
GRANT VIEW DEFINITION ON  [Batch].[CalculateMemberMonthDetail_v2] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[CalculateMemberMonthDetail_v2] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[CalculateMemberMonthDetail_v2] TO [Processor]
GO
