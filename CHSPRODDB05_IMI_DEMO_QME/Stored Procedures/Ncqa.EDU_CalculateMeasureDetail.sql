...
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			DELETE FROM Result.MeasureDetail_EDU WHERE BatchID = @BatchID AND DataRunID = @DataRunID AND DataSetID = @DataSetID AND MetricID = @MetricEDU;

			IF NOT EXISTS(SELECT TOP 1 1 FROM Result.MeasureDetail_EDU)
				TRUNCATE TABLE Result.MeasureDetail_EDU;

			INSERT INTO Result.MeasureDetail_EDU
					(BatchID,
					DataRunID,
					DataSetID,
					DSEntityID,
			...
			SELECT	RMD.BatchID,
					...
					RMD.DSEntityID,
					RMD.DSMemberID,
					CONVERT(decimal(18,12), ROUND((EXP(HCC4.TotalWeight)) / (1 + EXP(HCC4.TotalWeight)) * HCC5.TotalWeight, 4)) AS ExpectedQty,
					RMD.MetricID,
					CONVERT(decimal(18,12), ROUND((EXP(HCC4.TotalWeight)) / (1 + EXP(HCC4.TotalWeight)), 4)) AS Ppv,
			...
			FROM	Result.MeasureDetail AS RMD
					INNER JOIN Result.MeasureDetail_EDU AS EDU
							ON EDU.SourceRowGuid = RMD.ResultRowGuid AND
								EDU.SourceRowID = RMD.ResultRowID
			WHERE	(RMD.BatchID = @BatchID) AND
					(RMD.DataRunID = @DataRunID) AND
					(RMD.DataSetID = @DataSetID) AND
					(RMD.MetricID = @MetricEDU);						
...
GO
GRANT VIEW DEFINITION ON  [Ncqa].[EDU_CalculateMeasureDetail] TO [db_executer]
GO
GRANT EXECUTE ON  [Ncqa].[EDU_CalculateMeasureDetail] TO [db_executer]
GO
