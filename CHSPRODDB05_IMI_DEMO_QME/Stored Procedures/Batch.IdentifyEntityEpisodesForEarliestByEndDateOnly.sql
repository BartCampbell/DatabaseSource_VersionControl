...
-- Description:	Identifies the earliest possible entity episode per member by "End Date" only.
--				(Entity Type: 0B095280-D913-4878-A332-9773FFA101E2)
-- =============================================
CREATE PROCEDURE [Batch].[IdentifyEntityEpisodesForEarliestByEndDateOnly]
(
	@BatchID int,
		...
		IF 1=1--@BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Temp.EntityBase)
			BEGIN;
					...
					FROM	Measure.Entities AS ME
							...
							INNER JOIN Measure.EntityTypes AS MET
									ON ME.EntityTypeID = MET.EntityTypeID AND
										...
										MET.EntityTypeGuid = '0B095280-D913-4878-A332-9773FFA101E2'	
				)
				...
				SELECT	TEB.DSMemberID, TEB.EntityID, 
						COALESCE(MIN(CASE WHEN PE.IsSupplemental = 0 AND PE.IsNonSuppPrioritized = 1 THEN TEB.EndDate END), MIN(TEB.EndDate)) AS KeyDate, 
						MIN(TEB.EndDate) AS KeyDateNoSupplementalFilter,
						PE.SourceLinkID
				INTO	#KeyEntities
...
GO
GRANT VIEW DEFINITION ON  [Batch].[IdentifyEntityEpisodesForEarliestByEndDateOnly] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForEarliestByEndDateOnly] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForEarliestByEndDateOnly] TO [Processor]
GO
