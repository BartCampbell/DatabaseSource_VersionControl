...
-- Description:	Peforms the initial data gathering for calculating continuous enrollment for entities based on End Date (or Begin Date when missing).
--				(Date Comparer: 861FC054-B1A0-4EE6-B806-A05C4227A1FE)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityEnrollmentByEndDateIndexDOBSeed]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '861FC054-B1A0-4EE6-B806-A05C4227A1FE') AND
		...
		SELECT DISTINCT
				...
				DATEADD(dd, EEK.AnchorDays, DATEADD(mm, EEK.AnchorMonths, ISNULL(TEB.EndDate, TEB.BeginDate))) AS AnchorDate,
				DATEADD(dd, EEK.BeginDOBDays, DATEADD(mm, EEK.BeginDOBMonths, ISNULL(TEB.EndDate, TEB.BeginDate))) AS BeginDOBDate, 
				DATEADD(dd, EEK.BeginDays, DATEADD(mm, EEK.BeginMonths, ISNULL(TEB.EndDate, TEB.BeginDate))) AS BeginEnrollDate,
				DATEADD(dd, EEK.EndDOBDays, DATEADD(mm, EEK.EndDOBMonths, ISNULL(TEB.EndDate, TEB.BeginDate))) AS EndDOBDate,
				DATEADD(dd, EEK.EndDays, DATEADD(mm, EEK.EndMonths, ISNULL(TEB.EndDate, TEB.BeginDate))) AS EndEnrollDate,
				EEK.EntityID, EEK.GapDays, EEK.Gaps, EEK.Gender, EEK.MeasEnrollID, EEK.OptionNbr,
				DATEADD(dd, EEK.PayerDays, DATEADD(mm, EEK.PayerMonths, ISNULL(TEB.EndDate, TEB.BeginDate))) AS PayerDate,
				EEK.ProductClassID
		...
		WHERE	(TEB.RankOrder = 1) AND
				(TM.DOB BETWEEN DATEADD(dd, EEK.BeginDOBDays, DATEADD(mm, EEK.BeginDOBMonths, ISNULL(TEB.EndDate, TEB.BeginDate))) AND 
								DATEADD(dd, EEK.EndDOBDays, DATEADD(mm, EEK.EndDOBMonths, ISNULL(TEB.EndDate, TEB.BeginDate)))) AND
		...
		INSERT INTO Proxy.EntityEnrollment
				...
				MeasEnrollID, OptionNbr, PayerDate, ProductClassID)
		SELECT DISTINCT
				t.AdminGapDays, 
				t.AdminGaps, 
				t.AnchorDate,
...
GO
GRANT VIEW DEFINITION ON  [Batch].[InitializeEntityEnrollmentByEndDateIndexDOBSeed] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityEnrollmentByEndDateIndexDOBSeed] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityEnrollmentByEndDateIndexDOBSeed] TO [Processor]
GO
