...
-- Description:	Applies additional event base criteria  member and date. (v2)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEventBaseTypeD]
(
	@BatchID int
)
AS
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			INTO	#EventKey
			FROM	Proxy.EventKey 
			WHERE	(EventTypeID = @EventTypeD) AND
			...
			IF EXISTS(SELECT TOP (1) 1 AS N FROM #EventKey)
				BEGIN;
					...
					CREATE CLUSTERED INDEX IX_#InitEventBase ON #InitEventBase (DSMemberID, EndDate, BeginDate, EventID, OptionNbr);
...
GO
GRANT VIEW DEFINITION ON  [Batch].[ApplyEventBaseTypeD] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeD] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeD] TO [Processor]
GO
