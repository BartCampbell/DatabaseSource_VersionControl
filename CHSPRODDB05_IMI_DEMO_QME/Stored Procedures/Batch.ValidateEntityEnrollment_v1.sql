...
-- =============================================
CREATE PROCEDURE [Batch].[ValidateEntityEnrollment_v1]
(
	@BatchID int,
		...
		IF @BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Proxy.EntityBase)
			BEGIN;
				...
				SELECT	BenefitID,
						...
						MAX(LastSegBeginDate) AS LastSegBeginDate,
						MAX(LastSegEndDate) AS LastSegEndDate,
						MeasEnrollID,
						OptionNbr,
						ProductClassID,
				...
				WHERE	(ActualBeginDate <= TEN.EndEnrollDate) --Added WHERE to combat bad data in AAB Deck, 11/9/2016.  (Overlapping enrollment segments, exposed by remove of ProductlassID filtering)
				GROUP BY BenefitID,
						EntityBaseID, 
						EntityID,
						MeasEnrollID,
				...
				FROM	#Enrollment AS t
						...
						/***************************************************/
				GROUP BY B.CountBenefits, t.EntityBaseID, t.EntityID, t.MeasEnrollID, t.OptionNbr 
				...
				CREATE UNIQUE CLUSTERED INDEX IX_#ValidEnrolls ON #ValidEnrolls (EntityID, OptionNbr, MeasEnrollID, EntityBaseID);
				...
				GROUP BY N.CountEnrolls, t.EntityBaseID, t.EntityID, t.OptionNbr, N.MaxEnrollGaps, 
						CASE WHEN N.IgnoreClass = 1 THEN 0 ELSE PEK.ProductClassID END --Added as part of the "HasPayer" code
				...
				CREATE UNIQUE CLUSTERED INDEX IX_#EntityEligible ON #EntityEligible (EntityBaseID);
				...
				CREATE UNIQUE CLUSTERED INDEX IX_#ExtendSegments ON #ExtendSegments (EntityBaseID);       
				...
				IF @CalculateXml = 1 
					BEGIN;
						...
						FROM	Proxy.EntityBase AS PEB
								INNER JOIN #EntityEligible AS t
