...
-- Description:	Identifies the most recent possible entity episode per member by "End Date" only.
--				(Entity Type: 9846A6EC-685E-4BD1-BAA7-40821D74213B)
-- =============================================
CREATE PROCEDURE [Batch].[IdentifyEntityEpisodesForMostRecentByEndDateOnly]
(
	@BatchID int,
		...
		IF 1=1--@BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Temp.EntityBase)
			BEGIN;
					...
					FROM	Measure.Entities AS ME
							...
							INNER JOIN Measure.EntityTypes AS MET
									ON ME.EntityTypeID = MET.EntityTypeID AND
										...
										MET.EntityTypeGuid = '9846A6EC-685E-4BD1-BAA7-40821D74213B'	
				)
				...
				SELECT	TEB.DSMemberID, TEB.EntityID, 
						COALESCE(MAX(CASE WHEN PE.IsSupplemental = 0 AND PE.IsNonSuppPrioritized = 1 THEN TEB.EndDate END), MAX(TEB.EndDate)) AS KeyDate,
						MAX(TEB.EndDate) AS KeyDateNoSupplementalFilter,
						PE.SourceLinkID
				INTO	#KeyEntities
				...
				FROM	#PossibleEntities AS t
						...
						INNER JOIN #KeyEntities AS KE
								ON TEB.DSMemberID = KE.DSMemberID AND
									TEB.EndDate IS NOT NULL AND 
									TEB.EndDate = KE.KeyDate AND
...
GO
GRANT VIEW DEFINITION ON  [Batch].[IdentifyEntityEpisodesForMostRecentByEndDateOnly] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForMostRecentByEndDateOnly] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForMostRecentByEndDateOnly] TO [Processor]
GO
