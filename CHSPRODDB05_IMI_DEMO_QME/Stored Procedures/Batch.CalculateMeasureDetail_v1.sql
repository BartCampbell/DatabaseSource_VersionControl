...
-- =============================================
CREATE PROCEDURE [Batch].[CalculateMeasureDetail_v1]
(
	@BatchID int
)
AS
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			SELECT DISTINCT
					ME.AgeAsOfDays, ME.AgeAsOfMonths,
					METMM.EntityID, MM.MeasureID, MM.MeasureXrefID, 
					METMM.MetricID, MX.MetricXrefID, MMPL.ProductLineID 
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#MetricEligiblePopulationKey ON #MetricEligiblePopulationKey (EntityID, MetricID, MeasureID, ProductLineID);
			...
			INSERT INTO Result.MeasureDetail
					(Age, AgeMonths, BatchID, BeginDate, BitProductLines, DataRunID, DataSetID, DSEntityID, DSMemberID, 
					DSProviderID, EndDate, EnrollGroupID, EntityID, Gender, IsDenominator, IsIndicator, IsExclusion,
					IsNumerator, IsNumeratorAdmin, IsNumeratorMedRcd, KeyDate, MeasureID, MeasureXrefID, MetricID, MetricXrefID,
					PayerID, PopulationID, ProductLineID, Qty, ResultTypeID, SourceDenominator)
			SELECT DISTINCT
					...
					TE.BatchID, 
					TE.BeginDate, 
					ISNULL(PBP.BitProductLines, 0) AS BitProductLines,
					...
					ISNULL(PPPL.ProductLineID, 0) AS ProductLineID, 
					0 AS Qty, 
					1 AS ResultTypeID,
			...
			FROM	Proxy.Entities AS TE
					...
					LEFT OUTER JOIN Product.PayerProductLines AS PPPL
							ON PEK.PayerID = PPPL.PayerID AND
								t.ProductLineID = PPPL.ProductLineID 
			WHERE	(
						(TE.EnrollGroupID IS NULL) OR 
						(
							(PEK.EnrollGroupID  IS NOT NULL) AND
							(PPPL.ProductLineID IS NOT NULL) AND
							(PBP.PayerID IS NOT NULL)
						)
					)
			GROUP BY t.AgeAsOfDays, t.AgeAsOfMonths, 
					TE.BatchID, TE.BeginDate, PBP.BitProductLines, TM.DOB, TE.DSEntityID, TE.DSMemberID,
					TE.EndDate, TE.EnrollGroupID, TE.EntityID, TM.Gender,
					t.MeasureID, t.MeasureXrefID, t.MetricID, t.MetricXrefID, PEK.PayerID, 
					PEK.PopulationID, PPPL.ProductLineID
			ORDER BY MeasureID, MetricID, DSMemberID, KeyDate, ProductLineID;
			...
			FROM	Proxy.Entities AS PE
					...
					INNER JOIN Proxy.Enrollment AS MN
			...
			FROM	#Counters AS C
					...
					INNER JOIN Product.PayerProductLines AS PPPL
							ON PEK.PayerID = PPPL.PayerID AND
								MMPL.ProductLineID = PPPL.ProductLineID
					INNER JOIN #PayerBitProductLines AS PBP
							ON PEK.PayerID = PBP.PayerID;
			...
			WITH MetricCount AS
			(
				SELECT  COUNT(DISTINCT CASE WHEN MX.IsInverse = 0 THEN MX.MetricID END) AS CountMetrics,
						MX.MeasureID
			...
			SELECT	COUNT(DISTINCT CASE WHEN MX.IsInverse = 0 THEN MX.MetricID END) AS CountMetrics, METMM.EntityID, 
			...
			HAVING	(COUNT(DISTINCT CASE WHEN MX.IsInverse = 0 THEN MX.MetricID END) = X.CountMetrics) OR 
...
GO
GRANT VIEW DEFINITION ON  [Batch].[CalculateMeasureDetail_v1] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[CalculateMeasureDetail_v1] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[CalculateMeasureDetail_v1] TO [Processor]
GO
