...
-- =============================================
CREATE PROCEDURE [Cloud].[SubmitDataRunInfo_v1]
(
	@Data xml,
...
BEGIN
	...
	BEGIN TRY;
		...
		DECLARE	@UseSourceGuidAsMainGuid bit;
		SET @UseSourceGuidAsMainGuid = 1;
		...
		INTO	#DataSets
		FROM	Data AS d
				CROSS APPLY d.Data.nodes('/dataset') AS ds(dataset);
		...
		FROM	Data AS d
				CROSS APPLY d.Data.nodes('/dataset') AS ds(dataset)
				CROSS APPLY d.Data.nodes('/dataset/run') AS dr(datarun)
		...
		FROM	Data AS d
				CROSS APPLY d.Data.nodes('/dataset/run') AS dr(datarun)
				CROSS APPLY d.Data.nodes('/dataset/run/batch') AS b(batch)
		...
		SELECT	DS.CountClaimCodes,
				...
				DS.CreatedBy,
				DS.CreatedDate,
				--CASE WHEN @UseSourceGuidAsMainGuid = 1 THEN DS.SourceGuid ELSE NEWID() END,
				@EngineGuid,
				BDO.OwnerID,
				DS.SourceGuid
		FROM	#DataSets AS DS
		...
		SELECT	DR.[BatchSize],
				...
				DR.CreatedBy,
				DR.CreatedDate,
				CASE WHEN @UseSourceGuidAsMainGuid = 1 THEN DR.SourceGuid ELSE NEWID() END,
				DS.DataSetID,
		...
		INSERT INTO Batch.[Batches]
				...
				DataSetID,
				SourceGuid)
		SELECT	CASE WHEN @UseSourceGuidAsMainGuid = 1 THEN B.SourceGuid ELSE NEWID() END,
