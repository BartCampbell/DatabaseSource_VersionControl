...
-- Description:	Retrieves the list of code type groups.
-- =============================================
CREATE PROCEDURE [Report].[GetCodeTypeGroups]
(
	@AllowDefault bit = 1
)
AS
	...
	BEGIN TRY;
		...
		(
			SELECT	'(All)' AS Abbrev,
					'(All Code Groups)' AS Descr,
					CONVERT(tinyint, NULL) AS ID
			UNION
			SELECT	CCTG.Abbrev,
					CCTG.Descr,
					CCTG.CodeTypeGrpID
			FROM	Claim.CodeTypeGroups AS CCTG WITH(NOLOCK)
		)
		SELECT	t.Abbrev,
                t.Descr,
                t.ID
...
GO
GRANT EXECUTE ON  [Report].[GetCodeTypeGroups] TO [Reports]
GO
