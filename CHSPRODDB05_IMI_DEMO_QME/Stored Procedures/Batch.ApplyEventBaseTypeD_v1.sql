...
-- Description:	Populates the Temp.EventBase table with member and date event-based matching data. (v1)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEventBaseTypeD_v1]
(
	@BatchID int
)
AS
BEGIN
	...
	BEGIN TRY;
		...
		SET @LogObjectName = 'ApplyEventBaseTypeD'; 
		SET @LogObjectSchema = 'Batch'; 
		
		BEGIN TRY;
				
			IF @BatchID IS NULL
				RAISERROR(' - Applying additional member and date event-based details failed!  No batch was specified.', 16, 1);
			...
			--EVENT TYPE D: Matched on DSMemberID and BeginDate/EndDate-----------------------------------------------------
			IF EXISTS(SELECT TOP (1) 1 AS N FROM Proxy.EventKey WHERE EventTypeID = @EventTypeD)
				BEGIN;
					...
					WHERE	(CountCriteria > 1) AND
							...
							(EventTypeID = @EventTypeD);
							
					CREATE CLUSTERED INDEX IX_#EventBase ON #EventBase (DSMemberID, BeginDate, EndDate, EventID, OptionNbr) WITH (FILLFACTOR = 100);
					CREATE STATISTICS ST_#EventBase ON #EventBase (DSMemberID, BeginDate, EndDate, EventID, OptionNbr);
					...
					FROM	Proxy.ClaimCodes AS CCC
							...
							INNER JOIN #EventBase AS VB
									ON TEK.EventID = VB.EventID AND
										TEK.EventTypeID = @EventTypeD AND
			...
			SET @LogDescr = ' - Applying additional member and date event-based details for BATCH ' + ISNULL(CAST(@BatchID AS varchar), '?') + ' succeeded.'; 
		...
		BEGIN CATCH;
			...
			SET @LogEndTime = GETDATE();
			SET @LogDescr = ' - Applying additional member and date event-based details for BATCH ' + ISNULL(CAST(@BatchID AS varchar), '?') + ' refresh failed!'; --{FAILURE LOG DESCRIPTION HERE}
...
GO
GRANT VIEW DEFINITION ON  [Batch].[ApplyEventBaseTypeD_v1] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeD_v1] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeD_v1] TO [Processor]
GO
