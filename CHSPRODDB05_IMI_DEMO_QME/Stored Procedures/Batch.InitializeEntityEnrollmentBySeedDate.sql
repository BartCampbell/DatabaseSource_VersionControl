...
-- Description:	Peforms the initial data gathering for calculating continuous enrollment for entities based on Seed Date.
--				(Date Comparer: 6E7F9298-66D1-429F-8728-7BCF3CB295C7)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityEnrollmentBySeedDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '6E7F9298-66D1-429F-8728-7BCF3CB295C7') AND
		...
		SELECT DISTINCT
				...
				DATEADD(dd, EEK.AnchorDays, DATEADD(mm, EEK.AnchorMonths, @EndInitSeedDate)) AS AnchorDate,
				DATEADD(dd, EEK.BeginDOBDays, DATEADD(mm, EEK.BeginDOBMonths, @SeedDate)) AS BeginDOBDate, --DOB criteria always on Seed Date
				DATEADD(dd, EEK.BeginDays, DATEADD(mm, EEK.BeginMonths, @BeginInitSeedDate)) AS BeginEnrollDate,
				DATEADD(dd, EEK.EndDOBDays, DATEADD(mm, EEK.EndDOBMonths, @SeedDate)) AS EndDOBDate, --DOB criteria always on Seed Date
				DATEADD(dd, EEK.EndDays, DATEADD(mm, EEK.EndMonths, @EndInitSeedDate)) AS EndEnrollDate,
				EEK.EntityID, EEK.GapDays, EEK.Gaps, EEK.Gender, EEK.MeasEnrollID, EEK.OptionNbr,
				DATEADD(dd, EEK.PayerDays, DATEADD(mm, EEK.PayerMonths, @EndInitSeedDate)) AS PayerDate,
				EEK.ProductClassID
...
GO
GRANT VIEW DEFINITION ON  [Batch].[InitializeEntityEnrollmentBySeedDate] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityEnrollmentBySeedDate] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityEnrollmentBySeedDate] TO [Processor]
GO
