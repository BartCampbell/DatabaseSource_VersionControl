...
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			DELETE FROM Result.MeasureDetail_IHU WHERE BatchID = @BatchID AND DataRunID = @DataRunID AND DataSetID = @DataSetID AND MetricID IN (SELECT MetricID FROM #MetricKey);

			IF NOT EXISTS(SELECT TOP 1 1 FROM Result.MeasureDetail_IHU)
				TRUNCATE TABLE Result.MeasureDetail_IHU;

			INSERT INTO Result.MeasureDetail_IHU
					(BatchID,
					DataRunID,
					DataSetID,
					DSEntityID,
			...
			FROM	Result.MeasureDetail AS RMD
					INNER JOIN Result.MeasureDetail_IHU AS IHU
							ON IHU.SourceRowGuid = RMD.ResultRowGuid AND
								IHU.SourceRowID = RMD.ResultRowID
			WHERE	(RMD.BatchID = @BatchID) AND
					(RMD.DataRunID = @DataRunID) AND
					(RMD.DataSetID = @DataSetID);
...
GO
GRANT VIEW DEFINITION ON  [Ncqa].[IHU_CalculateMeasureDetail] TO [db_executer]
GO
GRANT EXECUTE ON  [Ncqa].[IHU_CalculateMeasureDetail] TO [db_executer]
GO
