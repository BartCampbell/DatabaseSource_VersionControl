...
-- Description:	Applies additional event base criteria by claim line. (v2)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEventBaseTypeL]
(
	@BatchID int,
	@IsClaimAttrib bit = 0
)
...
BEGIN
	...
	BEGIN TRY;
		...
		IF @IsClaimAttrib = 1
			SET @LogEntryXrefGuid = '29D2D948-5629-4C76-B7CE-517819EC2ED0';
		ELSE
			SET @LogEntryXrefGuid = '2AA2C167-3C36-4A4C-B228-1E978DDA834A';			
		-----------------------------------------------------------------------
		...
		BEGIN TRY;
			...
			INTO	#EventKey
			FROM	Proxy.EventKey 
			WHERE	(EventTypeID = @EventTypeL) AND
					(RankOrder > 1) AND
					(IsClaimAttrib = @IsClaimAttrib)
			OPTION (OPTIMIZE FOR (@IsClaimAttrib = 0));	;					
					
			CREATE UNIQUE CLUSTERED INDEX IX_#EventKey ON #EventKey (CodeID, BitSpecialtiesAllowed, BitSpecialtiesDenied, 
																		BitClaimAttribsAllowed, BitClaimAttribsDenied, BitClaimSrcTypesAllowed, 
																		BitClaimSrcTypesDenied, HasMemberReqs, EventCritCodeID, EventID, OptionNbr);
			...
			IF EXISTS(SELECT TOP (1) 1 AS N FROM Proxy.EventKey WHERE EventTypeID = @EventTypeL)
				BEGIN;
					...
					CREATE CLUSTERED INDEX IX_#InitEventBase ON #InitEventBase (DSClaimLineID, EventID, OptionNbr);
					...
					FROM	Internal.ClaimSource AS CS WITH(INDEX (IX_Internal_ClaimSource_CodeID))
							...
							INNER JOIN #InitEventBase AS VB
									ON TEK.EventID = VB.EventID AND
										...
										CS.DSClaimLineID = VB.DSClaimLineID
			...
			SET @LogDescr = ' - Applying additional claim line event-based details ' + CASE WHEN @IsClaimAttrib = 1 THEN 'of claim attributes ' ELSE '' END + 'for BATCH ' + ISNULL(CAST(@BatchID AS varchar), '?') + ' succeeded.'; 
		...
		BEGIN CATCH;
			...
			SET @LogEndTime = GETDATE();
			SET @LogDescr = ' - Applying additional claim line event-based details ' + CASE WHEN @IsClaimAttrib = 1 THEN 'of claim attributes ' ELSE '' END + 'for BATCH ' + ISNULL(CAST(@BatchID AS varchar), '?') + ' refresh failed!'; 
...
GO
GRANT VIEW DEFINITION ON  [Batch].[ApplyEventBaseTypeL] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeL] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[ApplyEventBaseTypeL] TO [Processor]
GO
