...
-- Description:	Identifies the most recent possible entity episode per member by "Begin Date".
--				(Entity Type: 010CECF7-D2DC-47CE-85FE-C114373C818A)
-- =============================================
CREATE PROCEDURE [Batch].[IdentifyEntityEpisodesForMostRecentByBeginDate]
(
	@BatchID int,
		...
		IF 1=1--@BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Temp.EntityBase)
			BEGIN;
					...
					FROM	Measure.Entities AS ME
							...
							INNER JOIN Measure.EntityTypes AS MET
									ON ME.EntityTypeID = MET.EntityTypeID AND
										...
										MET.EntityTypeGuid = '010CECF7-D2DC-47CE-85FE-C114373C818A'	
				)
				...
				SELECT	TEB.DSMemberID, TEB.EntityID, 
						COALESCE(MAX(CASE WHEN PE.IsSupplemental = 0 AND PE.IsNonSuppPrioritized = 1 THEN TEB.BeginDate END), MAX(TEB.BeginDate)) AS KeyDate, 
						MAX(TEB.BeginDate) AS KeyDateNoSupplementalFilter,
						PE.SourceLinkID
				INTO	#KeyEntities
