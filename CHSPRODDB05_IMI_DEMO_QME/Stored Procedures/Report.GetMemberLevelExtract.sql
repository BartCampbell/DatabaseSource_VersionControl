...
CREATE PROCEDURE [Report].[GetMemberLevelExtract]
    (
      @DataRunID INT ,
...
----------------------------------------------------------------------------------------------------
        ...
        --              OR ( RMD.ResultTypeID = 3 )
        --            );

        IF OBJECT_ID('PREF1_QINet_Staging.Export.MemberExtractExport') IS NOT NULL
            DROP TABLE PREF1_QINet_Staging.Export.MemberExtractExport;
        ...
        WITH    ResultTypes
            ...
            SELECT  DO.Descr AS Client ,
                    ...
                    RDSPK.Descr AS [ReportPopulation] ,
		...
		--RDSMGK.MedGrpName,
                    ...
                    CONVERT(INT, RMD.IsDenominator) AS Denominator ,
                    CONVERT(INT, RMD.IsExclusion) AS Exclusion ,
                    CONVERT(INT, CASE WHEN RMD.ResultTypeID = 3
                                      THEN CASE WHEN RMD.IsDenominator = 1
                                                THEN RMD.IsNumeratorAdmin
                                                ELSE 0
                                           END
                                 END) AS CompliantAdmin ,
                    CONVERT(INT, CASE WHEN RMD.ResultTypeID = 3
                                      THEN CASE WHEN RMD.IsDenominator = 1
                                                THEN RMD.IsNumeratorMedRcd
                                                ELSE 0
                                           END
                                 END) AS CompliantMedRcd ,
                    CONVERT(INT, CASE WHEN RMD.IsDenominator = 1
                                      THEN RMD.IsNumerator
                                      ELSE 0
                                 END) AS Compliant ,
                    CONVERT(INT, CASE WHEN RMD.IsDenominator = 1
                                      THEN CASE RMD.IsNumerator
                                             WHEN 1 THEN 0
                                             WHEN 0 THEN 1
                                           END
                                      ELSE 0
                                 END) AS [Non-Compliant] ,
                    CONVERT(INT, RMD.IsIndicator) AS Indicator ,
                    RMD.Age AS [Age] ,
                    CASE RMD.Gender
                      WHEN 0 THEN 'F'
                      WHEN 1 THEN 'M'
                    ...
                    END AS PCR_Variance
            --INTO    PREF1_QINet_Staging.Export.MemberExtractExport
            FROM    Result.MeasureDetail AS RMD
                    INNER JOIN #MeasureList AS t ON RMD.MeasureID = t.MeasureID
                                                    AND RMD.ResultTypeID = t.ResultTypeID
                    ...
                    LEFT OUTER JOIN Result.DataSetPopulationKey AS RDSPK ON RDSPK.DataRunID = RMD.DataRunID
...
GO
GRANT EXECUTE ON  [Report].[GetMemberLevelExtract] TO [Reports]
GO
