		...
		BEGIN TRY;
			...
			IF EXISTS(SELECT TOP 1 1 FROM #Risk_Base)
				BEGIN;   
					...
					SELECT	B.Age,
							B.AggregateID,
							CAST(0 AS decimal(18, 12)) AS BaseWeight,
							B.BitProductLines,
							...
							B.EvalTypeID,
							B.Gender,
							CONVERT(decimal(18,12), ROUND(
									CASE WHEN B.AggregateID = 1 
										 THEN SUM(ISNULL(CCW.[Weight], B.DefaultValue))
										 WHEN B.AggregateID = 2
										 THEN EXP(SUM(LOG(ISNULL(NULLIF(ABS(ISNULL(CCW.[Weight], B.DefaultValue)), 0), 1))))
										 ELSE ISNULL(MIN(B.DefaultValue), 0)
										 END, 4)
									) AS HClinCondWeight,
							B.MeasureID,
							B.MetricID,
