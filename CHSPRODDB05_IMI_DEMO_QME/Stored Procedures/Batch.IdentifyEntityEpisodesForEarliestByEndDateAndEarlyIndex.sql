...
-- Description:	Identifies the earliest possible entity episode per member by "End Date" with the episode chosen before eligibility is applied.
--				(Entity Type: 58FAF594-97E7-4968-B509-EAF494F15A66)
-- =============================================
CREATE PROCEDURE [Batch].[IdentifyEntityEpisodesForEarliestByEndDateAndEarlyIndex]
(
	@BatchID int,
		...
		IF 1 = 1--@BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Temp.EntityBase)
			BEGIN;
					...
					FROM	Measure.Entities AS ME
							...
							INNER JOIN Measure.EntityTypes AS MET
									ON ME.EntityTypeID = MET.EntityTypeID AND
										...
										MET.EntityTypeGuid = '58FAF594-97E7-4968-B509-EAF494F15A66'	
				)
				...
				SELECT	TEB.DSMemberID, TEB.EntityID, 
						COALESCE(MIN(CASE WHEN PE.IsSupplemental = 0 AND PE.IsNonSuppPrioritized = 1 THEN ISNULL(TEB.EndDate, TEB.BeginDate) END), MIN(ISNULL(TEB.EndDate, TEB.BeginDate))) AS KeyDate, 
						MIN(ISNULL(TEB.EndDate, TEB.BeginDate)) AS KeyDateNoSupplementalFilter,
						PE.SourceLinkID 
				INTO	#KeyEntities
				...
				FROM	#PossibleEntities AS t
						...
						INNER JOIN #KeyEntities AS KE 
								ON TEB.DSMemberID = KE.DSMemberID AND
									((TEB.EndDate = KE.KeyDate) OR 
									(TEB.EndDate IS NULL AND TEB.BeginDate = KE.KeyDate)) AND
...
GO
GRANT VIEW DEFINITION ON  [Batch].[IdentifyEntityEpisodesForEarliestByEndDateAndEarlyIndex] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForEarliestByEndDateAndEarlyIndex] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForEarliestByEndDateAndEarlyIndex] TO [Processor]
GO
