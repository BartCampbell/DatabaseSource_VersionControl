...
-- Description:	Identifies the most recent possible entity episode per member by "End Date" (or "Begin Date" when missing).
--				(Entity Type: 0487E1E0-2D54-4F7B-84F9-E2CF63097DEB)
-- =============================================
CREATE PROCEDURE [Batch].[IdentifyEntityEpisodesForMostRecentByEndDate]
(
	@BatchID int,
		...
		IF 1=1--@BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Temp.EntityBase)
			BEGIN;
					...
					FROM	Measure.Entities AS ME
							...
							INNER JOIN Measure.EntityTypes AS MET
									ON ME.EntityTypeID = MET.EntityTypeID AND
										...
										MET.EntityTypeGuid = '0487E1E0-2D54-4F7B-84F9-E2CF63097DEB'	
				)
				...
				SELECT	TEB.DSMemberID, TEB.EntityID, 
						COALESCE(MAX(CASE WHEN PE.IsSupplemental = 0 AND PE.IsNonSuppPrioritized = 1 THEN ISNULL(TEB.EndDate, TEB.BeginDate) END), MAX(ISNULL(TEB.EndDate, TEB.BeginDate))) AS KeyDate, 
						MAX(ISNULL(TEB.EndDate, TEB.BeginDate)) AS KeyDateNoSupplementalFilter,
						PE.SourceLinkID
				INTO	#KeyEntities
				...
				FROM	#PossibleEntities AS t
						...
						INNER JOIN #KeyEntities AS KE
								ON TEB.DSMemberID = KE.DSMemberID AND
									(((TEB.EndDate IS NOT NULL) AND (TEB.EndDate = KE.KeyDate)) OR
...
GO
GRANT VIEW DEFINITION ON  [Batch].[IdentifyEntityEpisodesForMostRecentByEndDate] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForMostRecentByEndDate] TO [db_executer]
GO
GRANT EXECUTE ON  [Batch].[IdentifyEntityEpisodesForMostRecentByEndDate] TO [Processor]
GO
