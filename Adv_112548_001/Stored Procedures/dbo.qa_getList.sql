...
BEGIN
	...
	SELECT DISTINCT Coded_User_PK INTO #tmp FROM tblSuspect
	SELECT User_PK,Lastname+', '+Firstname Fullname,location_pk FROM tblUser U INNER JOIN #tmp T ON T.Coded_User_PK=U.User_PK ORDER BY Fullname
	...
	SELECT @Location_PK = Location_PK,@IsAdmin=IsAdmin,@IsQAManager=IsQAManager FROM tblUser WHERE User_PK = @User_PK;
