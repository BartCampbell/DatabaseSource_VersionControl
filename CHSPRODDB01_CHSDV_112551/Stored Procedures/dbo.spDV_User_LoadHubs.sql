...
INSERT INTO [dbo].[H_Contact]
 ...
 Select 
		...
		Concat(RTRIM(LTRIM(COALESCE([sch_Tel],''))),RTRIM(LTRIM(COALESCE([sch_Fax],''))),RTRIM(LTRIM(COALESCE(Email_Address,'')))),
			...
			LoadDate 
	 FROM 
		CHSStaging.adv.tblUserStage rw
		where 
		upper(convert(char(32), HashBytes('MD5',
		Upper(Concat(
				RTRIM(LTRIM(COALESCE([sch_Tel],''))),':',
	...
	INSERT INTO H_Location
		...
		upper(convert(char(32), HashBytes('MD5',
			Upper(Concat(
				...
				LoadDate,
				RecordSource
		FROM CHSStaging.adv.tblUserStage
		where 
		upper(convert(char(32), HashBytes('MD5',
			Upper(Concat(
				RTRIM(LTRIM(COALESCE([Address],''))),':',
...
--** LOAD H_SESSION HUB FROM tblUserSessionStage


INSERT INTO [dbo].[H_Session]
           ([H_Session_RK]
           ,[Session_BK]
           ,[ClientSessionID]
		   ,[LoadDate]
...
SELECT	DISTINCT SessionHashKey, CSI, Session_PK,LoadDate,RecordSource
	FROM 
		CHSStaging.adv.tblUserSessionStage
	WHERE
		SessionHashKey not in (Select H_Session_RK from H_Session)
		AND CCI = @CCI
