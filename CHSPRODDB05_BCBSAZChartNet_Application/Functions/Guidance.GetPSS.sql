...
CREATE FUNCTION [Guidance].[GetPSS]
(	
	@PursuitEventID int
)
RETURNS table 
...
(
	...
	UNION
	SELECT	'First-Two-Enrolled-Prenatal Date Range' AS Title,
			dbo.ConvertDateToVarchar(t.FirstTwoEnrolledPrenatalStartDate) + 
				ISNULL(' (' + M1.HEDISMeasure + CASE WHEN t.FirstTwoEnrolledPrenatalStartSourceHasMultiMeasures = 1 THEN '+' ELSE '' END + ')', '') + ' - ' + 
			dbo.ConvertDateToVarchar(t.FirstTwoEnrolledPrenatalEndDate) +
				ISNULL(' (' + M2.HEDISMeasure + CASE WHEN t.FirstTwoEnrolledPrenatalEndSourceHasMultiMeasures = 1 THEN '+' ELSE '' END + ')', '') AS [Value],
			4.1 AS SortOrder
	FROM	ListBase AS t
			LEFT OUTER JOIN dbo.Measure AS M1
					ON M1.MeasureID = t.FirstTwoEnrolledPrenatalStartSourceMeasureID 
			LEFT OUTER JOIN dbo.Measure AS M2
					ON M2.MeasureID = t.FirstTwoEnrolledPrenatalEndSourceMeasureID
	WHERE	t.FirstTwoEnrolledPrenatalStartDate IS NOT NULL AND
			t.FirstTwoEnrolledPrenatalEndDate IS NOT NULL
	--UNION
