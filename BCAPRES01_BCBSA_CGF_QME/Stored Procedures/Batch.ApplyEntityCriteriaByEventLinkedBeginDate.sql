...
-- Description:	Applies the additional entity criteria based on the linked entity's Begin Date.  
--				(Date Comparer: 74457DBC-3C12-498D-B4A1-5787AA6EFA2E)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEntityCriteriaByEventLinkedBeginDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '74457DBC-3C12-498D-B4A1-5787AA6EFA2E') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				MEC.Allow, MEC.AllowActiveScript, MEC.AllowBeginDate, MEC.AllowEnrollSeg, MEC.AllowEndDate, MEC.AllowServDate,
				...
				MEC.OptionNbr, ISNULL(MEC.QtyMax, 32767) AS QtyMax, ISNULL(MEC.QtyMin, 1) AS QtyMin,
				MEC.RankOrder, MEC.RequireDiffProvider, MEC.RequirePaid, TE.DSEntityID AS SourceLinkID, MEC.ValueMax, MEC.ValueMin
		INTO	#Base
...
GO
GRANT EXECUTE ON  [Batch].[ApplyEntityCriteriaByEventLinkedBeginDate] TO [Processor]
GO
