...
-- Description:	Initializes entities with a top-ranked criteria between the linked entity's Begin Date and End Date.  
--				(Date Comparer: CFD50C88-5AC2-424E-8D86-E1A030430397)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityByEventLinkedBeginDateToEndDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = 'CFD50C88-5AC2-424E-8D86-E1A030430397') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				...
				DATEADD(dd, MEC.BeginDays, DATEADD(mm, MEC.BeginMonths, TE.BeginDate)) AS EntityBeginDate,
				MEC.EntityCritID, 
				DATEADD(dd, MEC.EndDays, DATEADD(mm, MEC.EndMonths, TE.EndDate)) AS EntityEndDate,
				...
				MEC.RankOrder, ISNULL(TE.EndDate, TE.BeginDate) AS SourceLinkEntityDate, TE.DSEntityID AS SourceLinkID, MEC.ValueMax, MEC.ValueMin
		INTO	#Base
...
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityByEventLinkedBeginDateToEndDate] TO [Processor]
GO
