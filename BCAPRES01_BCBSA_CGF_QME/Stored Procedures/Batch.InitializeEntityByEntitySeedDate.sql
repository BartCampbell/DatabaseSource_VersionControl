		...
		FROM	Measure.EntityCriteria  AS MEC
				...
				INNER JOIN Proxy.Members AS M
						ON TE.DSMemberID = M.DSMemberID AND
							...
							((MEC.DaysMax IS NOT NULL) AND (MEC.DaysMax >= DATEDIFF(dd, TE.BeginDate, ISNULL(TE.EndDate, TE.EndOrigDate)) + 1))) AND
							((MEC.DaysMin IS NULL) OR
							((MEC.DaysMin IS NOT NULL) AND (MEC.DaysMin <= DATEDIFF(dd, TE.BeginDate, ISNULL(TE.EndDate, TE.EndOrigDate)) + 1))) AND
