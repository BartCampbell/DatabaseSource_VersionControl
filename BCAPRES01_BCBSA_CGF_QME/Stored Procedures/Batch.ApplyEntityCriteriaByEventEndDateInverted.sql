...
-- Description:	Applies the additional entity criteria based on End Date (or Begin Date, when End Date is missing) of the originating event, where the targeted event is NOT within the specified range (e.g. inverted).  
--				(Date Comparer: B5BDD03C-E02F-4A4C-93D6-76542672593A)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEntityCriteriaByEventEndDateInverted]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = 'B5BDD03C-E02F-4A4C-93D6-76542672593A') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				...
				MEC.EntityCritID, 
				ISNULL(TEB.EndDate, TEB.BeginDate) AS EntityDate,
							...
							(
								(
									(
										...
										(TV.BeginDate NOT BETWEEN T.EntityBeginDate AND T.EntityEndDate)
									) OR
									(
										(t.AllowEndDate = 1) AND 
										(TV.EndDate NOT BETWEEN T.EntityBeginDate AND T.EntityEndDate)
									) OR
									(
