...
-- Description:	Determines the actual quantities for entity criteria. (v4)
-- =============================================
CREATE PROCEDURE [Batch].[QuantifyEntityCriteria_v4]
(
	@BatchID int,
	...
	BEGIN TRY;
		...
		IF @BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Proxy.EntityBase)
			BEGIN;
				...
				CREATE STATISTICS spIX_EntityOptions ON #EntityOptions (EntityID, OptionNbr);
				...
				CREATE STATISTICS spIX_#PossibleEntities ON #PossibleEntities (EntityBaseID);
				...
				CREATE STATISTICS spIX_#QtyLookupV1Of2 ON #QtyLookupV1Of2 
					--(DSMemberID, DateComparerInfo, DateCompTypeID, RowID);
					(DSMemberID, 
									DateComparerInfo, 
									DateCompTypeID, 
				...
				CREATE STATISTICS spIX_#QtyLookupV2Of2 ON #QtyLookupV2Of2 
					--(DSMemberID, DateComparerInfo, DateCompTypeID, RowID);
					(DSMemberID, 
									DateComparerInfo, 
									DateCompTypeID, 
				...
				CREATE STATISTICS spIX_#QtyLookupE1Of1 ON #QtyLookupE1Of1 (DSMemberID, DateComparerInfo, DateCompTypeID, RowID);
				...
				CREATE STATISTICS spIX_#EventLookup ON #EventLookup (DSMemberID, EventID);
				...
				CREATE STATISTICS spIX_#Events ON #Events
					-- (DSMemberID, EventID, DSEventID);
										(DSMemberID,
					EventID,
					BeginDate,
				...
				CREATE STATISTICS spIX_#Events2 ON #Events (EventID) 
