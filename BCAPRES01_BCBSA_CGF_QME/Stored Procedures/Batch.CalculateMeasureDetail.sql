...
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			--*Prereq: Denominator-to-Metric mapping list for use with nested entity matching (used on Ineligibility/Numerator/Indicator mappings)...
			DECLARE @i int; --WHILE loop counter
			DECLARE @MaxIteration int; --Reused variable to capture the maximum relevant iteration for the mapping type...
						
			SELECT DISTINCT
					EntityID, MetricID
			INTO	#EPEntityToMetricMapping
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#EPEntityToMetricMapping ON #EPEntityToMetricMapping (EntityID, MetricID);
			...
			--2c) Loop through nested-entities to match numerator records to the appropriate denominator entity
			...
			WHILE 1 = 1
				BEGIN;
					...
					FROM	#Ineligible AS t
							...
							LEFT OUTER JOIN #Ineligible AS x
									ON t.DSMemberID = x.DSMemberID AND
										...
										t.SourceIneligible = x.SourceIneligible
			...
			FROM	#Ineligible AS t
					INNER JOIN Proxy.Entities AS PE
							ON PE.DSEntityID = t.SourceIneligible
			...
			--2d) Update the measure result detail by zeroing denominator, numerator, and exclusion fields.	
				...
				BEGIN;
					...
					FROM	#Indicators AS t
							...
							LEFT OUTER JOIN #EPEntityToMetricMapping AS EP
									ON TE.EntityID = EP.EntityID AND
										t.MetricID = EP.MetricID
							LEFT OUTER JOIN #Indicators AS x
									ON t.DSMemberID = x.DSMemberID AND
										(TE.SourceLinkID = x.DSEntityID OR (TE.SourceLinkID IS NULL AND x.DSEntityID IS NULL)) AND
										t.MetricID = x.MetricID  AND
										t.SourceIndicator = x.SourceIndicator
					WHERE	(EP.MetricID IS NULL) AND  --Stop matching at the metric's eligible population entity, added 12/5/2012
							(x.DSMemberID IS NULL) --Prevent Duplicates;
			...
			FROM	#Indicators AS t
					INNER JOIN Proxy.Entities AS PE
							ON PE.DSEntityID = t.SourceIndicator
			WHERE	(t.IsSupplemental IS NULL);
			-------------------------------------------------------------------------------------------
			...
			FROM	#Result_MeasureDetail AS RMD
					INNER JOIN #Exclusions AS t
							ON RMD.DSMemberID = t.DSMemberID AND
								...
								(t.KeyMetricID IS NULL OR /*t.IsExcludeAll = 1 OR*/ RMD.MetricID = t.KeyMetricID) AND --Commented Out Section:  Removed for CIS ROTA exclusions on HEDIS 2017
								RMD.BatchID = @BatchID
			GROUP BY t.RowID
			...
			FROM	Proxy_Entities AS PE
					...
					OUTER APPLY (
									--Added 1/30/2017 for MPT/IAD to apply priority requirements...
									SELECT	MIN(tPN.[Priority]) AS [Priority]
									FROM	Internal.Enrollment AS tPN WITH(NOLOCK, INDEX(IX_Internal_Enrollment))
									WHERE	(tPN.DataSetID = M.DataSetID) AND 
											(tPN.DSMemberID = M.DSMemberID) AND
											(tPN.EnrollGroupID = tPN.EnrollGroupID) AND --Tricking the optimizer (possibly)
											(tPN.SpId = @@SPID) AND --Added since bypassing the Proxy view
											(ISNULL(PE.EndDate, PE.BeginDate) BETWEEN tPN.BeginDate AND tPN.EndDate) AND
											(
			...
			WHERE	(
					...
					(
						(MN.EnrollItemID IS NULL) OR --No active enrollment OR
						(MN.[Priority] = PN_Priority.[Priority]) --Best priority w/ active enrollment
					)
