...
-- Description:	Initializes entities with a top-ranked criteria based the linked entity's Original End Date (or Begin Date when missing).  
--				(Date Comparer: 1F8504FA-A338-4E60-95B9-699B7A6C76E2)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityByEventLinkedEndOrigDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '1F8504FA-A338-4E60-95B9-699B7A6C76E2') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				...
				DATEADD(dd, MEC.BeginDays, DATEADD(mm, MEC.BeginMonths, ISNULL(TE.EndOrigDate, TE.BeginOrigDate))) AS EntityBeginDate,
				MEC.EntityCritID, 
				DATEADD(dd, MEC.EndDays, DATEADD(mm, MEC.EndMonths, ISNULL(TE.EndOrigDate, TE.BeginOrigDate))) AS EntityEndDate,
