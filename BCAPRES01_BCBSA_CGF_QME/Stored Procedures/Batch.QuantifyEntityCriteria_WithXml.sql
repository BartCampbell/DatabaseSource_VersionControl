...
-- =============================================
CREATE PROCEDURE [Batch].[QuantifyEntityCriteria_WithXml]
(
	@BatchID int,
	...
	BEGIN TRY;
		...
		IF @BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Proxy.EntityBase)
			BEGIN;
				...
				SELECT @QtyComparerX = QtyComparerID FROM Measure.QuantityComparers WHERE Abbrev = 'X';
				...
				--2a) Identify entity base records requiring quantity comparison...
				...
				CREATE UNIQUE CLUSTERED INDEX IX_#QtyLookupBase ON #QtyLookupBase (DSMemberID, EntityCritID, EntityBeginDate, EntityEndDate, RowID);

				--2b) Consolidate quantity calculations to eliminate redundant counting on EntityBaseIDs that have the exact same criteria...
				SELECT DISTINCT
						AfterDOBDate,
                        Allow,
                        AllowActiveScript,
				...
				CREATE UNIQUE CLUSTERED INDEX IX#_QtyLookup ON #QtyLookup (DSMemberID, EntityCritID, EntityBeginDate, EntityEndDate);  --"Should be" the uniqueness of the previous DISTINCT (most fields are specified at the EntityCritID-level)

				--2c) Create a crosswalk of the consolidated lookups and their matching EntityBaseIDs (and ...EntityBase.RowIDs)
				SELECT	QLB.EntityBaseID,
						QLB.RowID AS EntityBaseRowID,
						QL.RowID
				INTO	#QtyLookupCrosswalk
				FROM	#QtyLookup AS QL WITH(INDEX(1))
						INNER JOIN #QtyLookupBase AS QLB WITH(INDEX(1))
								ON QLB.DSMemberID = QL.DSMemberID AND
									QLB.EntityCritID = QL.EntityCritID AND
									QLB.EntityBeginDate = QL.EntityBeginDate AND
									QLB.EntityEndDate = QL.EntityEndDate;

				DROP TABLE #QtyLookupBase;
				CREATE UNIQUE CLUSTERED INDEX IX_#QtyLookupCrosswalk ON #QtyLookupCrosswalk (RowID, EntityBaseRowID);
				
				--2d) Break QtyLookup into the three versions of comparisons to try to improve performance...
				...
				SELECT * INTO #QtyLookupV2Of2 FROM #QtyLookup AS QL WHERE QL.DateCompTypeID IN (@DateCompTypeV) AND QL.QtyComparerID IN (@QtyComparerS, @QtyComparerT, @QtyComparerU, @QtyComparerV, @QtyComparerW, @QtyComparerX);
				...
				SELECT	PV.BeginDate,
				        ...
				        PV.EndDate,
				        PV.EndOrigDate,
				        CASE WHEN @CalculateXml = 1 THEN (SELECT PV.EventInfo AS [quantities] FOR XML PATH(''), TYPE) END AS EntityQtyInfo,
						PV.EventBaseID,
				        PV.EventCritID,
				        PV.EventID,
				        PV.IsPaid,
				...
				WHERE	(
							...
							(
								(PV.EndDate IS NOT NULL) AND
								(PV.BeginDate < VL.EntityBeginDate) AND
								(PV.EndDate > VL.EntityEndDate)
							) OR
							(
								(PV.EndOrigDate IS NOT NULL) AND
								(VL.AllowActiveScript = 1) AND
								(PV.BeginDate < VL.EntityBeginDate) AND
								(PV.EndOrigDate > VL.EntityEndDate)
							)
						) AND
				...
				SELECT	COUNT(*) AS CountRecords,
						...
						CASE WHEN @CalculateXml = 1
							 THEN dbo.CombineXml(PV.EntityQtyInfo, 1)
							 END AS EntityQtyInfo,
						QL.RowID,
						CONVERT(decimal(12, 6), CASE QL.QtyComparerID 
							...
							WHEN @QtyComparerF THEN dbo.CountDates(ISNULL(PV.EndDate, PV.BeginDate), 14, 0)
							...
							END) AS Qty,
						CONVERT(decimal(12, 6), CASE QL.QtyComparerID 
							WHEN @QtyComparerC THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN PV.DSClaimID END)
							WHEN @QtyComparerD THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN REPLACE(CONVERT(char(16), PV.EventCritID) + CONVERT(char(16), PV.EventBaseID) + CONVERT(char(8), ISNULL(PV.DispenseID, 1)), ' ', '_') END)
							--WHEN @QtyComparerD THEN COUNT(DISTINCT REPLACE(CONVERT(char(16), PV.EventCritID) + CONVERT(char(8), PV.BeginDate, 112) + CONVERT(char(8), ISNULL(PV.DispenseID, 1)), ' ', '_'))
							WHEN @QtyComparerF THEN dbo.CountDates(CASE WHEN PV.IsSupplemental = 0 THEN ISNULL(PV.EndDate, PV.BeginDate) END, 14, 0)
							WHEN @QtyComparerG THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN ISNULL(PV.EndDate, PV.BeginDate) END)
							WHEN @QtyComparerK THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN PV.EventCritID END)
							WHEN @QtyComparerL THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN PV.DSClaimLineID END)
							WHEN @QtyComparerP THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN REPLACE(CONVERT(char(16), PV.DSProviderID) + CONVERT(char(8), ISNULL(PV.EndDate, PV.BeginDate), 112), ' ', '_') END)
							ELSE COUNT(DISTINCT PV.DSEventID)
							END) AS QtyNoSupplemental, --Added to capture non-supplemental totals for later setting IsSupplemental flag (based on copy of "Qty" field)
						QL.QtyComparerID
				INTO	#NewQuantities
				...
				WHERE	QL.DateCompTypeID = @DateCompTypeV AND
						QL.QtyComparerID IN (@QtyComparerS, @QtyComparerT, @QtyComparerU, @QtyComparerV, @QtyComparerW, @QtyComparerX)
				...
				SELECT	COUNT(*) AS CountRecords,
						...
						CONVERT(decimal(12, 6), CASE QL.QtyComparerID 
							...
							WHEN @QtyComparerX THEN dbo.CountDates(C.D, 1, 1)
							ELSE 0
							END) AS Qty,
						CONVERT(decimal(12, 6), CASE QL.QtyComparerID 
							WHEN @QtyComparerS THEN COUNT(CASE WHEN PV.IsSupplemental = 0 THEN C.D END)
							WHEN @QtyComparerT THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN C.D END)
							WHEN @QtyComparerU THEN COUNT(CASE WHEN PV.IsSupplemental = 0 THEN C.D END)
							WHEN @QtyComparerV THEN COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN C.D END)
							WHEN @QtyComparerW THEN ROUND
													(
														(
														CONVERT(decimal(18,9), COUNT(DISTINCT CASE WHEN PV.IsSupplemental = 0 THEN C.D END)) / 
														CONVERT(decimal(18,9), DATEDIFF(dd, MIN(QL.EntityBeginDate), MIN(CASE 
																															WHEN QL.EntityEndDate > @EndInitSeedDate 
																															THEN @EndInitSeedDate 
																															ELSE QL.EntityEndDate 
							...
							WHEN @QtyComparerX THEN dbo.CountDates(CASE WHEN PV.IsSupplemental = 0 THEN C.D END, 1, 1)
							ELSE 0
							END) AS QtyNoSupplemental, --Added to capture non-supplemental totals for later setting IsSupplemental flag (based on copy of "Qty" field)
						QL.QtyComparerID
				FROM	#QtyLookupV2Of2 AS QL WITH(INDEX (IX_#QtyLookupV2Of2))
						...
						INNER JOIN @Calendar AS C
								ON	(
									...
									(
										(QL.QtyComparerID IN (@QtyComparerS, @QtyComparerT, @QtyComparerW, @QtyComparerX) AND C.D BETWEEN QL.EntityBeginDate AND QL.EntityEndDate) OR
				...
				SELECT	PE.BeginDate,
				        ...
				        PE.EntityCritID,
				        PE.EntityID,
						CASE WHEN @CalculateXml = 1 THEN (SELECT PE.EntityInfo AS [quantities] FOR XML PATH(''), TYPE) END AS EntityQtyInfo,
						PE.IsSupplemental,
				        PE.SourceID,
				        PE.SourceLinkID
				INTO	#Entities
				...
				WHERE	(
							...
							(
								(PE.EndDate IS NOT NULL) AND
								(PE.BeginDate < VE.EntityBeginDate) AND
								(PE.EndDate > VE.EntityEndDate)
							)
						) AND
				...
				SELECT	COUNT(*) AS CountRecords,
						...
						CASE WHEN @CalculateXml = 1
							 THEN dbo.CombineXml(PE.EntityQtyInfo, 1)
							 END AS EntityQtyInfo,
						QL.RowID,
						CASE QL.QtyComparerID 
							WHEN @QtyComparerF THEN dbo.CountDates(ISNULL(PE.EndDate, PE.BeginDate), 14,0)
							...
							END AS Qty,
						CASE QL.QtyComparerID 
							WHEN @QtyComparerF THEN dbo.CountDates(CASE WHEN PE.IsSupplemental = 0 THEN ISNULL(PE.EndDate, PE.BeginDate) END, 14,0)
							WHEN @QtyComparerG THEN COUNT(DISTINCT CASE WHEN PE.IsSupplemental = 0 THEN ISNULL(PE.EndDate, PE.BeginDate) END)
							WHEN @QtyComparerP THEN COUNT(DISTINCT CASE WHEN PE.IsSupplemental = 0 THEN REPLACE(CONVERT(char(48), PE.DSProviderID) + CONVERT(char(48), ISNULL(PE.EndDate, PE.BeginDate), 109), ' ', '_') END)
							ELSE COUNT(DISTINCT CASE WHEN PE.IsSupplemental = 0 THEN PE.DSEntityID END)
							END AS QtyNoSupplemental, --Added to capture non-supplemental totals for later setting IsSupplemental flag (based on copy of "Qty" field)
						QL.QtyComparerID
				...
				IF @CalculateXml = 1
					UPDATE	t
					SET		EntityQtyInfo.modify('insert (
															attribute records { sql:column("t.CountRecords") }, 
															attribute qty { sql:column("t.Qty") }, 
															attribute qtyNonSupplemental { sql:column("t.QtyNoSupplemental") }, 
															attribute type { sql:column("MQC.Descr") },
															attribute typeId { sql:column("t.QtyComparerID") }
														) as first into (/quantities[1])')
					FROM	#NewQuantities AS t
							INNER JOIN Measure.QuantityComparers AS MQC
									ON MQC.QtyComparerID = t.QtyComparerID
					WHERE	(EntityQtyInfo IS NOT NULL);
				
				--4a) "Expand" the quantities out to the propery EntityBaseIDs (and ...EntityBase.RowIDs) via the crosswalk...
				SELECT	NQ.CountRecords,
                        NQ.DataSourceID,
						QLC.EntityBaseID,
                        NQ.EntityQtyInfo,
                        QLC.EntityBaseRowID AS RowID,
                        NQ.Qty,
						NQ.QtyNoSupplemental,
                        NQ.QtyComparerID
				INTO	#NewQuantitiesCrosswalk
				FROM	#NewQuantities AS NQ
						INNER JOIN #QtyLookupCrosswalk AS QLC
								ON QLC.RowID = NQ.RowID;

				CREATE UNIQUE CLUSTERED INDEX IX_#NewQuantitiesCrosswalk ON #NewQuantitiesCrosswalk (RowID);
				
				--4b) Apply quantities to the source data...
				UPDATE	PEB
				SET		DataSourceID = ISNULL(t.DataSourceID, PEB.DataSourceID),
						EntityQtyInfo = t.EntityQtyInfo,
						IsSupplemental = CASE WHEN 
												   --Either 1) Qty is all non-supplemental...
												   (t.QtyNoSupplemental = t.Qty) OR
												   --Or 2) Non-supplemental qty meets qty requirements...
												   (
														(
															(t.QtyNoSupplemental >= PEB.QtyMin) OR 
															(PEB.QtyMin IS NULL)
														) AND
														(
															(t.QtyNoSupplemental <= PEB.QtyMax) OR 
															(PEB.QtyMax IS NULL)
														)
												   )
											  THEN 0
											  ...
											  END,
						Qty = t.Qty,
						QtyNoSupplemental = t.QtyNoSupplemental
				FROM	Proxy.EntityBase AS PEB
						INNER JOIN #NewQuantitiesCrosswalk AS t
