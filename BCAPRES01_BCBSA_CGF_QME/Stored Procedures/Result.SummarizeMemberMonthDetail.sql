...
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;				
			...
			SELECT * INTO #Result_MemberMonthDetail_Classic FROM Result.MemberMonthDetail_Classic WHERE DataRunID = @DataRunID;

			CREATE UNIQUE CLUSTERED INDEX IX_#Result_MemberMonthDetail_Classic ON #Result_MemberMonthDetail_Classic (DataRunID, DSMemberID, PopulationID, ProductLineID, Age, BenefitID, 
																														EnrollGroupID, PayerID, ResultRowID);
			...
			SELECT	MIN(Age) AS Age, 
					...
					RMMD.ProductLineID
			INTO	#MemberAges
			FROM	#Result_MemberMonthDetail_Classic AS RMMD
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#MemberAges ON #MemberAges (DataRunID, DSMemberID, PopulationID, ProductLineID, Age, BenefitID, 
																			EnrollGroupID, PayerID);
			...
			SELECT	RMMD.Age,
					...
					RMMD.PopulationID,
					RMMD.ProductLineID
			FROM	#Result_MemberMonthDetail_Classic AS RMMD WITH(INDEX(1))
					LEFT OUTER JOIN #MemberAges AS t WITH(INDEX(1))
