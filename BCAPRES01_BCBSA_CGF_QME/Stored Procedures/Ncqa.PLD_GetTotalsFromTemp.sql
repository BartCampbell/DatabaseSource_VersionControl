...
BEGIN
	...
	SELECT	@SqlCmd =	ISNULL(@SqlCmd + ', ', '') +
						CASE WHEN COLUMN_NAME LIKE '%Numerator' OR 
							 ...
							 WHEN COLUMN_NAME IN ('RecordID', 'MemberID', 'DOB', 'Gender', 'DSMemberID', 'IhdsMemberID', 'Plan_ID', 'Product_Line', 'CIN', 'County', 'Metal_Level', 'Marketplace_Product', 'QARR-Age_Group')
			...
			@SqlCmdGroupBy  =	REPLACE(ISNULL(@SqlCmdGroupBy + ', ', '') +
								CASE WHEN COLUMN_NAME LIKE '%Numerator' OR 
										  ...
										  COLUMN_NAME IN ('RecordID', 'MemberID', 'DOB', 'Gender', 'DSMemberID', 'IhdsMemberID', 'Plan_ID', 'Product_Line', 'CIN', 'County', 'Metal_Level', 'Marketplace_Product', 'QARR-Age_Group')
									 THEN '<<BLANK>>'
									 ELSE QUOTENAME(COLUMN_NAME)
									 END, ', <<BLANK>>', '')
