...
BEGIN
	...
	SELECT DISTINCT
			...
			--'		ALTER INDEX ALL ON ' + QUOTENAME(C.TABLE_SCHEMA) + '.' + QUOTENAME(C.TABLE_NAME) + ' DISABLE;' + CHAR(13) + CHAR(10) + 
			'		UPDATE ' + QUOTENAME(C.TABLE_SCHEMA) + '.' + QUOTENAME(C.TABLE_NAME) + ' SET [SPID] = ' + CONVERT(nvarchar(max), @SPID) + ' WHERE ([SPID] <> ' + CONVERT(nvarchar(max), @SPID) + ');' + CHAR(13) + CHAR(10) +
			--'		ALTER INDEX ALL ON ' + QUOTENAME(C.TABLE_SCHEMA) + '.' + QUOTENAME(C.TABLE_NAME) + ' REBUILD;' + CHAR(13) + CHAR(10) + 
			'	END' + CHAR(13) + CHAR(10) AS Cmd,
