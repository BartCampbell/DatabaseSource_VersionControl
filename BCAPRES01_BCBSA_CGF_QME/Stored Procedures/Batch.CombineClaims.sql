...
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			DECLARE @ClaimAttribLAB smallint;	--Lab
			...
			SELECT @ClaimAttribLAB = ClaimAttribID FROM Claim.Attributes WHERE Abbrev = 'LAB';
			...
			DECLARE @BitClaimAttribLAB bigint;	--Lab
			...
			SELECT @BitClaimAttribLAB = BitValue FROM Claim.Attributes WHERE Abbrev = 'LAB';
			...
			SELECT	CCL.BeginDate,
					...
					MAX(CONVERT(tinyint, CASE WHEN CCA.ClaimAttribID = @ClaimAttribLAB THEN 1 ELSE 0 END)) AS IsLAB,
			...
			DELETE FROM #ClaimLinesBase WHERE IsLAB = 1; --Added HEDIS 2017, used as part of the replacement of POS-81-hard-coding for ignoring diagnosis codes on encounter-based lab claims. 
