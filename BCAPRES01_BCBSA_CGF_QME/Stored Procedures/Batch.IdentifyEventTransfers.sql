	...
	DECLARE @Result int;
		...
		BEGIN TRY;
			...
			(
				...
				DSProviderID bigint NULL, --Added as part of same-provider transfers, updated to allow NULL 2/14/2017
			...
			(
				...
				DSMemberID bigint NOT NULL,
				DSProviderID bigint NULL, --updated to allow NULL 2/14/2017
			...
			WHILE 1 = 1
				...
				BEGIN
					INSERT INTO #Transfers
							(DSMemberID,
							DSProviderID, --Added as part of same-provider transfers
					...
					SELECT	TV.DSMemberID,
							CONVERT(int, CASE WHEN MVT.MatchOnDSProviderID = 1 THEN TV.DSProviderID ELSE NULL END) AS DSProviderID, --Added as part of same-provider transfers
							...
							CASE WHEN MVT.IsPharmacyOnly = 1 THEN TV.EndOrigDate ELSE ISNULL(TV.EndDate, TV.BeginDate) END AS FromEndDate,
					...
					FROM	Internal.[Events] AS TV WITH(INDEX(IX_Internal_Events))
							INNER JOIN #Measure_EventTransfers AS MVT WITH(INDEX(1))
									ON TV.EventID = MVT.FromEventID AND
										(
											--(TV.EndDate IS NOT NULL) OR --Removed 10/26/2016 for SPR
											(MVT.IsPharmacyOnly = 0) OR --Added 10/26/2016 for SPR
											(
							...
							INNER JOIN Internal.[Events] AS t WITH(INDEX(IX_Internal_Events))
									ON MVT.ToEventID = t.EventID AND
										...
										TV.SpId = t.SpId AND
										(MVT.MatchOnDSProviderID = 0 OR TV.DSProviderID = t.DSProviderID) AND --Added as part of same-provider transfers
										...
										(
											...
											(	--Added 10/26/2016 for SPR certification for HEDIS 2017, observation stays
												MVT.IsPharmacyOnly = 0 AND
												t.BeginDate BETWEEN DATEADD(mm, MVT.BeginMonths, DATEADD(dd, MVT.BeginDays, TV.BeginDate)) AND 
																	DATEADD(mm, MVT.EndMonths, DATEADD(dd, MVT.EndDays, TV.BeginDate)) AND
												TV.BeginDate < t.EndDate AND
												TV.EndDate IS NULL 
												--------------------------------------------------------------------------
											) OR
											(
					...
					GROUP BY X.DSMemberID, X.DSProviderID, X.EventCritID, X.FromEventID, C.D;
					
					--Clustered index required for "Quirky Update".  (Do No Remove)
					CREATE UNIQUE CLUSTERED INDEX IX_#DateRanges ON #DateRanges (DSMemberID, EventID, DSProviderID, EventCritID, EvalDate); -- Added DSProviderID 02/15/2017
					
					DECLARE @LastDSMemberID bigint;
					DECLARE @LastDSProviderID bigint;
					...
					SET		@RangeID = CASE WHEN DSMemberID <> ISNULL(@LastDSMemberID, -1) OR 
												 ...
												 (EventCritID IS NOT NULL AND ISNULL(EventCritID, -1) <> ISNULL(@LastEventCritID, -1)) OR
												 (DSProviderID IS NOT NULL AND ISNULL(DSProviderID, -1) <> ISNULL(@LastDSProviderID, -1))
											THEN 1
					...
					SELECT		MIN(EvalDate) AS BeginDate,
								...
								MAX(XferID) AS XferID		
					FROM		#DateRanges
					GROUP BY	DSMemberID, DSProviderID, EventCritID, EventID, RangeID;
					
					CREATE UNIQUE CLUSTERED INDEX IX_#TransferOverrideDates ON #TransferOverrideDates (DSMemberID ASC, EventID ASC, BeginDate ASC, DSProviderID ASC, EventCritID ASC); -- DSProviderID added 02152017
					...
					FROM	Internal.[Events] AS PV WITH(INDEX(IX_Internal_Events))
							...
							INNER JOIN #TransferOverrideDates AS X WITH(INDEX(1))
									ON PV.DSMemberID = X.DSMemberID AND
										(X.DSProviderID IS NULL OR PV.DSProviderID = X.DSProviderID) AND
										...
										(
											...
											(
												...
												PV.BeginDate BETWEEN X.BeginDate AND X.EndDate
												--------------------------------------------------------------------------
											) OR
