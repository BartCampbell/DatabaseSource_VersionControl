	...
	BEGIN TRY;
		...
		*/
		...
		SELECT	PC.DateDispensed AS BeginDate,
				...
				NULL AS CPTMod2,
				NULL AS CPTMod3,
				NULLIF(LTRIM(RTRIM(PC.CVX)), '') AS CVX,
				...
				NULL AS LabValue,
				NULL AS LOINC,
				NULLIF(LTRIM(RTRIM(PC.NDC)), '') AS NDC,
				NULL AS POS,
		...
		INSERT INTO Claim.ClaimCodes --CVX
				(ClaimTypeID, Code, CodeTypeID, DataSetID, DSClaimLineID, DSMemberID, IsPrimary)
		SELECT	@ClaimTypeP,
				LTRIM(RTRIM(PC.CVX)) AS Code,
				@CodeTypeV AS CodeTypeID,
				@DataSetID AS DataSetID,
				CL.DSClaimLineID,
				CL.DSMemberID,
				0 AS IsPrimary
