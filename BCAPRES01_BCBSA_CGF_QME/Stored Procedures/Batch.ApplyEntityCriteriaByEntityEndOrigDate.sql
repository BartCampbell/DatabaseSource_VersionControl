...
-- Description:	Applies the additional entity criteria based on the original End Date (or Begin Date, when End Date is missing) of the originating entity.  
--				(Date Comparer: 99580254-B9D7-466A-8CD0-CDCC573361E9)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEntityCriteriaByEntityEndOrigDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '99580254-B9D7-466A-8CD0-CDCC573361E9') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				...
				MEC.DaysMax, MEC.DaysMin, TEB.DSMemberID, TEB.DSProviderID, 
				DATEADD(dd, MEC.BeginEnrollSegDays, DATEADD(mm, MEC.BeginEnrollSegMonths, ISNULL(TEB.EndOrigDate, TEB.BeginOrigDate))) AS EnrollSegBeginDate,
				DATEADD(dd, MEC.EndEnrollSegDays, DATEADD(mm, MEC.EndEnrollSegMonths, ISNULL(TEB.EndOrigDate, TEB.BeginOrigDate))) AS EnrollSegEndDate,
				TEB.EntityBaseID,
				DATEADD(dd, MEC.BeginDays, DATEADD(mm, MEC.BeginMonths, ISNULL(TEB.EndOrigDate, TEB.BeginOrigDate))) AS EntityBeginDate,
				MEC.EntityCritID, 
				DATEADD(dd, MEC.EndDays, DATEADD(mm, MEC.EndMonths, ISNULL(TEB.EndOrigDate, TEB.BeginOrigDate))) AS EntityEndDate,
		...
		FROM	#Base AS t
				INNER JOIN Proxy.Entities AS TE
						ON t.DateComparerInfo = TE.EntityID AND
							...
							((t.DaysMax IS NOT NULL) AND (t.DaysMax >= DATEDIFF(dd, TE.BeginDate, ISNULL(TE.EndDate, TE.EndOrigDate)) + 1))) AND
							((t.DaysMin IS NULL) OR
							((t.DaysMin IS NOT NULL) AND (t.DaysMin <= DATEDIFF(dd, TE.BeginDate, ISNULL(TE.EndDate, TE.EndOrigDate)) + 1))) AND
							
							--Apply Enrollment Segment Criteria-------------------------------------------------------
							((t.EnrollSegBeginDate IS NULL) OR (t.EnrollSegBeginDate <= TE.LastSegBeginDate)) AND
							((t.EnrollSegEndDate IS NULL) OR (t.EnrollSegEndDate >= TE.LastSegBeginDate))
...
GO
GRANT EXECUTE ON  [Batch].[ApplyEntityCriteriaByEntityEndOrigDate] TO [Processor]
GO
