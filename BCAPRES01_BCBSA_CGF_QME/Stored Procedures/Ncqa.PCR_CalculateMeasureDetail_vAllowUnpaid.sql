...
-- =============================================
CREATE PROCEDURE [Ncqa].[PCR_CalculateMeasureDetail_vAllowUnpaid]
(
	@BatchID int = NULL,
...
BEGIN
	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			DECLARE @PCR_MeasureID int;
			DECLARE @PCR_MetricID int;
			DECLARE @PCRSurg_MetricID int;
			
			SELECT @PCR_MeasureID = MeasureID FROM Measure.Measures WHERE (Abbrev = 'PCR') AND (MeasureSetID = @MeasureSetID);
			SELECT @PCR_MetricID = MetricID FROM Measure.Metrics WHERE (Abbrev = 'PCR') AND (MeasureID = @PCR_MeasureID);
			SELECT @PCRSurg_MetricID = MetricID FROM Measure.Metrics WHERE (Abbrev = 'PCRSurg') AND (MeasureID = @PCR_MeasureID);
			...
			--Added as part of 2017 breaking out surgeries for better planned stay tracking...
			SELECT	RMD.DSEntityID,
					RMD.DSMemberID,
					RMD.IsIndicator,
					RMD.ResultRowID
			INTO	#PCR_Surgeries
			...
			WHERE	(RMD.BatchID = @BatchID) AND
					...
					(RMD.IsIndicator = 1) AND
					(RMD.MeasureID = @PCR_MeasureID) AND
					(RMD.MetricID = @PCRSurg_MetricID);	

			CREATE UNIQUE CLUSTERED INDEX IX_#PCR_Surgeries ON #PCR_Surgeries (DSMemberID, DSEntityID);
			...
			SELECT	RMD.Age,
					...
					RMD.EnrollGroupID,
					RMD.Gender,
					RMD.IsIndicator AS HasFirstPlannedStay,
					ISNULL(Surg.IsIndicator, 0) AS HasSurgery,
					P.ProductLineID,
			...
			FROM	Result.MeasureDetail AS RMD
					--Added as part of 2017 breaking out surgeries for better planned stay tracking...
					LEFT OUTER JOIN #PCR_Surgeries AS Surg 
							ON Surg.DSEntityID = RMD.DSEntityID AND
								Surg.DSMemberID = RMD.DSMemberID
			...
			GROUP BY 
					...
					RMD.ResultRowGuid,
					RMD.ResultRowID
			ORDER BY DSMemberID, EndDate, ResultRowID;
			...
			IF EXISTS (SELECT TOP 1 1 FROM #PCR_Base)
				BEGIN;
					...
					--Step #10, Log "HCCs"
					DELETE FROM [Log].PCR_ClinicalConditions WHERE BatchID = @BatchID AND MeasureID = @PCR_MeasureID;
