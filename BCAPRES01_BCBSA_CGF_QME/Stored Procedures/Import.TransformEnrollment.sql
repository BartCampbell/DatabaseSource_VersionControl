...
BEGIN
	...
	BEGIN TRY;
		...
		INSERT INTO Member.Enrollment
				(BeginDate, BitProductLines, DataSetID, DataSourceID, DSMemberID, EndDate, EligibilityID, EnrollGroupID, IsEmployee, [Priority])
		SELECT
				ISNULL(E.DateEffective, @DefaultEnrollDate) AS BeginDate,
				MAX(CASE 
						WHEN E.CoverageHospiceFlag = 'Y' AND @ApplyHospiceProductLineFilter = 1 /*--> @ApplyHospiceProductLineFilter added 10/12/2016. Hospice requirements applied differently, starting HEDIS 2017 (General Guideline 20)*/ 
						THEN G.BitProductLines ^ @BitMedicare & G.BitProductLines ELSE G.BitProductLines 
						END) AS BitProductLines, --BitProductlines added 12/18/2013 for HEDIS 2014 and new hospice requirement (General Guideline 7)
				@DataSetID,
				...
				MIN(ISNULL(E.ProductPriority, G.[Priority])) AS [Priority]
