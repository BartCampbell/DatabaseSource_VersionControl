...
BEGIN
	...
	(
		...
		WHERE	(C.IsEnabled = 1) AND
				...
				((@HybridOnly = 0) OR (MRT.ResultTypeID IN (2, 3))) AND
				(
					(@PayerID IS NULL) OR
					(DE.PayerID IS NULL) OR
					(DE.PayerID = @PayerID)
				)           
	),
	Step2 AS
	...
	BEGIN;
		...
		(
			SELECT	SUM(CountMembers) AS CountMembers,
					...
					DE.MeasureSetID,
					--RMS.PopulationID,
					0 AS PopulationID, --Replaced with dummy population for HEDIS 2017's TLM spec changes
					RMS.ProductLineID,
					DE.ProductTypeID
			...
			WHERE	(RMS.DataRunID = @DataRunID) --AND 
					...
					--(RMS.PopulationID = @PopulationID) --Commented out for HEDIS 2017's TLM spec changes
