...
-- Description:	Initializes entities with a top-ranked criteria based the linked entity's Last Enrollment Segment's Begin Date and End Date.  
--				(Date Comparer: 505671A7-EB64-4EB2-9914-096B35AAA81C)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityByEventLinkedServDateToLastSegEndDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '505671A7-EB64-4EB2-9914-096B35AAA81C') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				MEC.Allow, MEC.AllowActiveScript, MEC.AllowBeginDate, MEC.AllowEndDate, MEC.AllowEnrollSeg, MEC.AllowServDate,
				...
				MEC.EntityCritID, 
				CASE WHEN DATEADD(dd, MEC.EndDays, DATEADD(mm, MEC.EndMonths, TE.LastSegEndDate)) > @EndInitSeedDate THEN @EndInitSeedDate ELSE DATEADD(dd, MEC.EndDays, DATEADD(mm, MEC.EndMonths, TE.LastSegEndDate)) END AS EntityEndDate,
				...
				MEC.RankOrder, TE.DSEntityID AS SourceLinkID, MEC.ValueMax, MEC.ValueMin
		INTO	#Base
		...
		SELECT	t.Allow, 
				...
				TV.EndDate,
				TV.EndOrigDate,
				CAST(NULL AS bigint) AS EnityBaseID,
				t.EntityBeginDate,
				t.EntityCritID, 
				t.EntityEndDate,
				t.EntityID, 
						...
						ON t.DateComparerInfo = TV.EventID AND
							...
							(
								...
								(	
									...
									(t.EntityBeginDate > TV.BeginDate) AND
									(t.EntityBeginDate BETWEEN TV.BeginDate AND TV.EndOrigDate)
								)
							...
							((t.DaysMax IS NOT NULL) AND (t.DaysMax >= DATEDIFF(dd, TV.BeginDate, ISNULL(TV.EndDate, TV.EndOrigDate)) + 1))) AND
							((t.DaysMin IS NULL) OR
							((t.DaysMin IS NOT NULL) AND (t.DaysMin <= DATEDIFF(dd, TV.BeginDate, ISNULL(TV.EndDate, TV.EndOrigDate)) + 1))) AND
							...
							((t.ValueMin IS NOT NULL) AND (t.ValueMin <= TV.Value)))  AND
							
							--Apply Enrollment Segment Limitation on Matched Entity/Event Key Date--------------------
							((t.AllowEnrollSeg = 1) OR (t.LastSegBeginDate <= TV.BeginDate) OR (t.LastSegBeginDate <= TV.EndDate))
...
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityByEventLinkedServDateToLastSegEndDate] TO [Processor]
GO
