	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			SELECT	MAX(CASE WHEN EventID = @DenominatorEventID THEN CountConsecutive ELSE 0 END) AS CountDays,
				  ...
				  -- Changed per HEDIS 2017 Numerator paid changes. As well as adding 34 day transfer to Numerator event like Denominator event.
					MAX(CASE WHEN EventID = @NumeratorEventID  THEN CountConsecutiveTwoOrMore ELSE 0 END) AS CountDaysTwoOrMore,
					ResultRowID
