		...
		IF @BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Proxy.EntityBase)
			BEGIN;
				...
				INSERT INTO #Enrollment
				        ...
				        ProductClassID,
				        TotGaps)
				SELECT	PEN.BenefitID,
						ISNULL(MIN(CASE WHEN PEN.ActualHasPayer = 1 AND PEN.ActualEnrollGroupID IS NOT NULL THEN PEN.BitProductLines END), MIN(PEN.BitProductLines)) AS BitProductLines,
						ISNULL(MIN(CASE WHEN PEN.ActualHasPayer = 1 THEN PEN.ActualEnrollGroupID END), MIN(PEN.ActualEnrollGroupID)) AS EnrollGroupID,
						PEN.EntityBaseID, 
						PEN.EntityID,
						MAX(PEN.ActualHasAnchor) AS HasAnchor,
						CASE WHEN COUNT(DISTINCT CASE WHEN PEN.ActualHasPayer = 1 THEN PEN.ActualEnrollGroupID END) > 1 THEN 1 ELSE 0 END AS HasMultiplePayers,
						MAX(PEN.ActualHasPayer) AS HasPayer,
						ISNULL(MAX(PEN.LastSegBeginDate), MAX(CASE WHEN PEN.ActualHasPayer = 1 THEN PEN.EnrollSegBeginDate END)) AS LastSegBeginDate,
						ISNULL(MAX(PEN.LastSegEndDate), MAX(CASE WHEN PEN.ActualHasPayer = 1 THEN PEN.EnrollSegEndDate END)) AS LastSegEndDate,
						PEN.MeasEnrollID,
						PEN.OptionNbr,
						ISNULL(MIN(CASE WHEN PEN.ActualHasPayer = 1 THEN PEN.[Priority] END), MIN(PEN.[Priority])) AS [Priority],
						PEN.ProductClassID,
						MAX(PEN.ActualGaps) AS TotGaps
				FROM	Proxy.EntityEnrollment AS PEN
				WHERE	(PEN.ActualBeginDate <= PEN.EndEnrollDate) --Required for current concurrent, dual-eligible enrollment segments logic. --Commented out on 10/13/2015 as part of HEDIS 2017 changes.
				GROUP BY PEN.BenefitID,
						PEN.EntityBaseID, 
						PEN.EntityID,
						PEN.MeasEnrollID,
						PEN.OptionNbr,
						PEN.ProductClassID
				HAVING	SUM(PEN.ActualGaps) <= MAX(PEN.Gaps) AND
						SUM(PEN.ActualGapDays) <= MAX(PEN.GapDays) AND
						SUM(PEN.ActualGapMaxDays) <= MAX(PEN.GapDays);
				...
				SELECT	PEN.BenefitID,
						MIN(PEN.BitProductLines) AS BitProductLines,
						PEN.ActualEnrollGroupID AS EnrollGroupID,
						PEN.EntityBaseID, 
						PEN.EntityID,
						PEN.MeasEnrollID,
						PEN.OptionNbr,
						MIN(PEN.[Priority]) AS [Priority],
						PEN.ProductClassID,
						MAX(PEN.ActualGaps) AS TotGaps
				INTO	#AdditionalEnrollGroups
				FROM	Proxy.EntityEnrollment AS PEN
				WHERE	(PEN.ActualHasPayer = 1) AND
						(PEN.ActualEnrollGroupID IS NOT NULL)
				GROUP BY PEN.ActualEnrollGroupID,
						PEN.BenefitID,
						PEN.EntityBaseID, 
						PEN.EntityID,
						PEN.MeasEnrollID,
				...
				--1c) Fix priority and gap count on the primary record, if needed, because of MIN on Step #1a
				UPDATE	N
				SET		[Priority] = NGK.[Priority],
						[TotGaps] = NGK.[TotGaps]
				...
				WHERE	(N.EnrollGroupID = NGK.EnrollGroupID) AND
						(
							(N.[Priority] <> NGK.[Priority]) OR
							(N.TotGaps <> NGK.TotGaps)
						);

				--1d) Insert the dual-eligible enrollment groups that don't already exist in #Enrollment...
				INSERT INTO #Enrollment
				        (BenefitID,
				        BitProductLines,
				        EnrollGroupID,
				...
				GROUP BY B.CountBenefits, 
						...
						t.MeasEnrollID, 
						t.OptionNbr;
				--HAVING (COUNT(DISTINCT t.BenefitID) >= B.CountBenefits) --Removed as part of Benefit filtering moved to earlier in the continuous enrollment process for HEDIS 2017
				...
				SELECT 	--ISNULL(MIN(CASE WHEN t.HasPayer = 1 THEN t.BitProductLines END), MIN(t.BitProductLines)) AS BitProductLines,
						...
						--MAX(t.LastSegEndDate) AS LastSegEndDate,
						t.OptionNbr,
						ISNULL(MIN(CASE WHEN t.HasPayer = 1 THEN t.[Priority] END), MIN(t.[Priority])) AS [Priority]
				INTO	#ValidOptions
				...
				FROM	#ValidOptions AS VO
						INNER JOIN #ValidEnrolls AS VN
								ON VN.EntityBaseID = VO.EntityBaseID AND
									...
									VN.OptionNbr = VO.OptionNbr AND
									VN.[Priority] = VO.[Priority]
