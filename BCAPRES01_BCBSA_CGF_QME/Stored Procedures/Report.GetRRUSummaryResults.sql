...
CREATE PROCEDURE [Report].[GetRRUSummaryResults]
(
	@DataRunID INT = NULL,
...
BEGIN
		...
		WITH RRUAgeBands(Measure, FromAge, ToAge, Abbrev) AS
		(
				  SELECT 'RDI', 18, 44, '1844'
			UNION SELECT 'RDI', 45, 54, '4554'
			UNION SELECT 'RDI', 55, 64, '5564'
			UNION SELECT 'RDI', 65, 75, '6575'
			UNION SELECT 'RCA', 18, 44, '1844'
			UNION SELECT 'RCA', 45, 54, '4554'
			UNION SELECT 'RCA', 55, 64, '5564'
			UNION SELECT 'RCA', 65, 75, '6575'
			UNION SELECT 'RCO', 42, 44, '4244'
			UNION SELECT 'RCO', 45, 64, '4564'
			UNION SELECT 'RCO', 65, 74, '6574'
			UNION SELECT 'RCO', 75, 255, '75+'
			UNION SELECT 'RAS',  5, 17, '0517'
			UNION SELECT 'RAS', 18, 44, '1844'
			UNION SELECT 'RAS', 45, 54, '4554'
			UNION SELECT 'RAS', 55, 64, '5564'
			UNION SELECT 'RHY', 18, 44, '1844'
			UNION SELECT 'RHY', 45, 54, '4554'
			UNION SELECT 'RHY', 55, 64, '5564'
			UNION SELECT 'RHY', 65, 85, '6585'
		)
		...
		FROM	Result.MeasureDetail AS RMD WITH(NOLOCK)
				...
				INNER JOIN Measure.MeasureClasses AS MMC WITH(NOLOCK)
						ON MM.MeasClassID = MMC.MeasClassID AND
							MMC.Abbrev = 'RRU'              
				LEFT OUTER JOIN Result.MeasureDetail_RRU AS RRU WITH(NOLOCK)
				...
				LEFT OUTER JOIN Ncqa.RRU_RiskCategories AS NRRC WITH(NOLOCK)
						ON RRU.RiskCtgyID = NRRC.RiskCtgyID
		WHERE	RMD.DataRunID = @DataRunID AND
				RMD.PopulationID = @PopulationID AND
				(@MetricID IS NULL OR MX.MetricID = @MetricID) AND
		...
		SELECT  Measure,
				MIN(MeasureDescr) AS MeasureDescr,
				MeasureID,
				SUM(CostInpatientCapped) AS [Cost - Inpatient],
				SUM(CostEMInpatientCapped) [Cost - E & M, Inpatient],
				SUM(CostEMOutpatientCapped) AS [Cost - E & M, Outpatient],
				SUM(CostProcInpatientCapped) AS [Cost - Procedure, Inpatient],
				SUM(CostProcOutpatientCapped) AS [Cost - Procedure, Outpatient],
				SUM(CostPharmacyCapped) AS [Cost - Pharmacy],
				SUM(CostImagingCapped) AS [Cost - Imaging],
				SUM(CostLabCapped) AS [Cost - Lab],
				SUM(FreqProcCABG) AS [Frequency - Procedure, CABG],
				SUM(FreqProcCAD) AS [Frequency - Procedure, CAD],
				SUM(FreqProcCardiacCath) AS [Frequency - Cardiac Cath],
				SUM(FreqProcCAT) AS [Frequency - CAT],
				SUM(FreqProcCAS) AS [Frequency - CAS],
				SUM(FreqProcEndarter) AS [Frequency - Endarter],
				SUM(FreqProcPCI) AS [Frequency - PCI],
				SUM(FreqED) AS [Frequency - ED],
				SUM(DaysAcuteInpatientNotSurg) AS [Days - Inpatient, Medicine],
				SUM(FreqAcuteInpatientNotSurg) AS [Qty - Inpatient, Medicine],
				SUM(DaysAcuteInpatientSurg) AS [Days - Inpatient, Surgery],
				SUM(FreqAcuteInpatientSurg) AS [Qty - Inpatient, Surgery],
				SUM(DaysNonacuteInpatient) AS [Days - Inpatient, Nonacute],
				SUM(FreqNonacuteInpatient) AS [Qty - Inpatient, Nonacute],
				SUM(MM) AS [Mbr Months - Med],
				SUM(MMP) AS [Mbr Months - Pharm],
				SUM(FreqPharmG1) AS [Frequency - Pharm, Generic Only],
				SUM(FreqPharmG2) AS [Frequency - Pharm, Generic/Name Brand Available],
				SUM(FreqPharmN1) AS [Frequency - Pharm, Name Brand Only],
				SUM(FreqPharmN2) AS [Frequency - Pharm, Name Brand/Generic Available],
				SUM(IsDenominator) AS Denominator,
				SUM(IsIndicator) AS Exclusions
		FROM	#RRUIdssBase 
		GROUP BY Measure, MeasureID
...
GO
GRANT EXECUTE ON  [Report].[GetRRUSummaryResults] TO [Reports]
GO
