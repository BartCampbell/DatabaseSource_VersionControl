...
-- Description:	Applies the additional entity criteria based on the original Date of Service of the originating entity.  
--				(Date Comparer: 9878C511-0521-46C2-BAAA-CF4EEF53E141)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEntityCriteriaByEntityServDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '9878C511-0521-46C2-BAAA-CF4EEF53E141') AND
		...
		FROM	#Base AS t
				INNER JOIN Proxy.Entities AS TE
						ON t.DateComparerInfo = TE.EntityID AND
							...
							((t.DaysMin IS NOT NULL) AND (t.DaysMin <= DATEDIFF(dd, TE.BeginDate, ISNULL(TE.EndDate, TE.EndOrigDate)) + 1)))
...
GO
GRANT EXECUTE ON  [Batch].[ApplyEntityCriteriaByEntityServDate] TO [Processor]
GO
