...
BEGIN
	...
	DECLARE @Result int;
		...
		BEGIN TRY;
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#ValidEventBase ON #ValidEventBase (EventTypeID, EventBaseID, IsSummarized, IsSupplemental);
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#KeyEventBase ON #KeyEventBase (EventBaseID, IsSupplemental);
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#Events ON #Events (EventID, EventCritID, BeginDate, DSMemberID, EndDate, ClaimTypeID, EventBaseID);
			...
			SELECT DISTINCT
					...
					MV.ForceBeginDate
			INTO	#EventKey
			FROM	Proxy.EventKey AS K
					INNER JOIN Measure.Events AS MV
							ON MV.EventID = K.EventID; 
			...
			FROM	#EventKey AS VK WITH(INDEX(IX_#EventKey))
					INNER JOIN #Events AS V WITH(INDEX(IX_#Events))
							ON VK.EventID = V.EventID AND	
								...
								(
									(VK.ForceBeginDate = 0) OR
									(
										VK.ForceBeginDate = 1 AND
										VK.BeginDate <= V.BeginDate
									)
								)
