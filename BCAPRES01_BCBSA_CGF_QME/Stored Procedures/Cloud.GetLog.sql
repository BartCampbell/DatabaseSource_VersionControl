				...
				IF (EXISTS (SELECT TOP 1 1 FROM @ProcessEntries)) OR 
					...
					(SELECT	ISNULL(batch.SourceGuid, batch.BatchGuid) AS id,
							(
								...
								FROM	Log.ProcessEntries AS LPE WITH(NOLOCK)
							...
							(
								...
								FROM	Log.ProcessErrors AS LPR WITH(NOLOCK)
										INNER JOIN Log.ProcessEntries AS LPE WITH(NOLOCK)
												ON LPR.LogID = LPE.ErrLogID
								WHERE	(LPE.BatchID = batch.BatchID) AND
										(
					...
					FROM	Batch.[Batches] AS batch WITH(NOLOCK)
