...
-- =============================================
CREATE PROCEDURE [Batch].[CalculateMemberMonthDetail]
(
				...
				--Create a valid enrollment key for calculating member months...
                    ...
                    SELECT	/*Member.GetAgeAsOf(M.DOB, t.EvalDate) AS Age,
							...
							MAX(CAST(DATEDIFF(YEAR, M.DOB, t.EvalDate) -
								CASE WHEN (MONTH(M.DOB) > MONTH(t.EvalDate)) OR
										  ((DAY(M.DOB) > DAY(t.EvalDate)) AND
										  (MONTH(M.DOB) = MONTH(t.EvalDate))) THEN 1
							...
							CASE WHEN ((DAY(M.DOB) > DAY(t.EvalDate)) AND
									   (MONTH(M.DOB) = MONTH(t.EvalDate))
									  ) THEN 1
								 ELSE 0
							END AS smallint)) AS AgeMonths,
							...
							----------------------------------------------------------------------------
							----------------------------------------------------------------------------
							MN.[Priority] AS [Priority]
							----------------------------------------------------------------------------
							----------------------------------------------------------------------------
					INTO	#Months
                    FROM    Proxy.Enrollment AS MN
							
                            INNER JOIN Proxy.Members AS M ON MN.DSMemberID = M.DSMemberID
                            INNER JOIN #MemberMonthDates AS t ON t.EvalDate BETWEEN MN.BeginDate AND MN.EndDate
					...
					-------------------------------------------------------------------------------------------------------------------------
					-- 2017 Identify overlapping enrollments with lower priority
					-------------------------------------------------------------------------------------------------------------------------
					IF OBJECT_ID('tempdb..#PriorityOverlap') IS NOT NULL
						DROP TABLE #PriorityOverlap;
					SELECT M.ID
					INTO #PriorityOverlap
					FROM    #Months M
					INNER JOIN #Months IM ON IM.DSMemberID = M.DSMemberID AND
                                 IM.Age = M.Age AND
                                 IM.MbrMonthDefID = M.MbrMonthDefID AND
                                 IM.Priority <> M.Priority AND
                                 IM.EnrollGroupID <> M.EnrollGroupID AND
                                 M.Priority NOT IN (
                                 SELECT MIN(Priority)
                                 FROM   #Months MPB
                                 WHERE  MPB.DSMemberID = M.DSMemberID AND 
										MPB.Age = M.Age AND
										MPB.EvalDate = M.EvalDate AND
										MPB.MbrMonthDefID = M.MbrMonthDefID)
					GROUP BY M.ID;
					-------------------------------------------------------------------------------------------------------------------------
					-- 2017 Remove overlapping enrollments with lower priority
					-------------------------------------------------------------------------------------------------------------------------		
					DELETE #Months FROM #Months M
					INNER JOIN #PriorityOverlap O ON O.ID = M.ID;	
					-------------------------------------------------------------------------------------------------------------------------
					-------------------------------------------------------------------------------------------------------------------------
					-------------------------------------------------------------------------------------------------------------------------							
					...
					SELECT	T.Age AS Age, 
							ISNULL(MAX(t.AgeMonths),0) AS AgeMonths,
							@BatchID AS BatchID, 
							t.BitBenefits,
					...
					WHERE	t.Age BETWEEN 0 AND 255 AND
							(
