...
-- Description:	Applies the additional entity criteria between the Begin Date and End Date (only) of the originating entity.  
--				(Date Comparer: F85A0131-89B0-4E34-B4F9-1D218643B84B)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEntityCriteriaByEntityBeginDateToEndDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = 'F85A0131-89B0-4E34-B4F9-1D218643B84B') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				...
				DATEADD(dd, MEC.BeginEnrollSegDays, DATEADD(mm, MEC.BeginEnrollSegMonths, TEB.EndDate)) AS EnrollSegBeginDate,
				DATEADD(dd, MEC.EndEnrollSegDays, DATEADD(mm, MEC.EndEnrollSegMonths, TEB.EndDate)) AS EnrollSegEndDate,
				TEB.EntityBaseID,
...
GO
GRANT EXECUTE ON  [Batch].[ApplyEntityCriteriaByEntityBeginDateToEndDate] TO [Processor]
GO
