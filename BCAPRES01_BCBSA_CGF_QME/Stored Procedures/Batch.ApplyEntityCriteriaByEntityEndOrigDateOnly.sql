...
-- Description:	Applies the additional entity criteria based on the original End Date (only) of the originating entity.  
--				(Date Comparer: 44C03650-81B8-4CB5-8CD4-551CD70D1AE3)
-- =============================================
CREATE PROCEDURE [Batch].[ApplyEntityCriteriaByEntityEndOrigDateOnly]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '44C03650-81B8-4CB5-8CD4-551CD70D1AE3') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				...
				DATEADD(dd, MEC.BeginEnrollSegDays, DATEADD(mm, MEC.BeginEnrollSegMonths, TEB.EndOrigDate)) AS EnrollSegBeginDate,
				DATEADD(dd, MEC.EndEnrollSegDays, DATEADD(mm, MEC.EndEnrollSegMonths, TEB.EndOrigDate)) AS EnrollSegEndDate,
				TEB.EntityBaseID,
				DATEADD(dd, MEC.BeginDays, DATEADD(mm, MEC.BeginMonths, TEB.EndOrigDate)) AS EntityBeginDate,
				MEC.EntityCritID, 
				DATEADD(dd, MEC.EndDays, DATEADD(mm, MEC.EndMonths, TEB.EndOrigDate)) AS EntityEndDate,
...
GO
GRANT EXECUTE ON  [Batch].[ApplyEntityCriteriaByEntityEndOrigDateOnly] TO [Processor]
GO
