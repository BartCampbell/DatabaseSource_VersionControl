...
-- Description:	Purges all tables with data related to the specified data run.
-- =============================================
CREATE PROCEDURE [Batch].[PurgeDataRun]
(
...
BEGIN
	...
	BEGIN TRY;			
		...
		SET @ConfCodeTableName = '##PurgeDataRunConfirmationCode_' + DB_NAME() + '_SPID' + CONVERT(nvarchar(128), @@SPID);
		...
		WHERE	(
				...
				(C.COLUMN_NAME IN ('BatchID', 'DataRunID')) AND
				(T.TABLE_TYPE = 'BASE TABLE')
		GROUP BY C.TABLE_SCHEMA,
				C.TABLE_NAME
		HAVING	(MAX(CASE WHEN C.COLUMN_NAME = 'DataRunID' THEN 1 ELSE 0 END) = 1)
		...
		WHILE (@ID BETWEEN @MinID AND @MaxID)
			BEGIN;
				...
				IF @Confirmed = 1
					BEGIN;
						SET @SqlCmd = 'DELETE FROM ' + @TableObject + ' WHERE (DataRunID = ' + CONVERT(nvarchar(max), @DataRunID) + ')';
						EXEC (@SqlCmd);
				...
				ELSE
					BEGIN;
						SET @SqlCmd = 'SELECT COUNT(*) AS CountRecords FROM ' + @TableObject + ' WHERE (DataRunID = ' + CONVERT(nvarchar(max), @DataRunID) + ')';
		...
		ELSE
			BEGIN;
				...
				UPDATE Batch.DataRuns SET IsPurged = 1, PurgedDate = GETDATE() WHERE DataRunID = @DataRunID;
			END;
