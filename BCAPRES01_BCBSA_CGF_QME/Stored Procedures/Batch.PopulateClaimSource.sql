	...
	BEGIN TRY;
		...
		BEGIN TRY;
			...
			SELECT	PCL.BatchID,
					...
					CONVERT(bit, CASE WHEN PCL.ClaimTypeID = @ClaimTypeE AND /*PCC.CodeTypeID IN (@ICD9DiagCodeType, @ICD10DiagCodeType) AND PCL.CPT LIKE '8____' AND*/ PCL.POS = '81' THEN 1 ELSE 0 END) AS IsLab, --Per NCQA, changed back to earlier logic 2/19/2014 for RRU.  On 11/13/2015, commented out 80000 CPT req due to URI.  On 11/3/2016, commented out code type requirements per NCQA PCS response.  
