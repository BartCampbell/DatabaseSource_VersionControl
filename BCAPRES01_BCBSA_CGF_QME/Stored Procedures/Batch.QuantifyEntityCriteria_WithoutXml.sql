...
-- =============================================
CREATE PROCEDURE [Batch].[QuantifyEntityCriteria_WithoutXml]
(
	@BatchID int,
	...
	BEGIN TRY;
		...
		IF @BatchID IS NOT NULL AND	@Iteration IS NOT NULL AND EXISTS (SELECT TOP 1 1 FROM Proxy.EntityBase)
			BEGIN;
				...
				FROM	#QtyLookupV1Of2 AS QL WITH(INDEX (IX_#QtyLookupV1Of2))
						INNER JOIN #Events AS PV WITH(INDEX (IX_#Events))
								ON QL.DSMemberID = PV.DSMemberID AND
									...
									((QL.DaysMax IS NOT NULL) AND (QL.DaysMax >= DATEDIFF(dd, PV.BeginDate, ISNULL(PV.EndDate, PV.EndOrigDate)) + 1))) AND
									((QL.DaysMin IS NULL) OR
									((QL.DaysMin IS NOT NULL) AND (QL.DaysMin <= DATEDIFF(dd, PV.BeginDate, ISNULL(PV.EndDate, PV.EndOrigDate)) + 1))) AND
				...
				--WHERE	QL.QtyComparerID IN (@QtyComparerS, @QtyComparerT, @QtyComparerU, @QtyComparerV, @QtyComparerW, @QtyComparerX)
				GROUP BY QL.DSMemberID,
						QL.QtyComparerID,
						QL.RowID
				OPTION (FORCE ORDER);
				...
				--4) Update quantities in associated entity base records...
				IF @Ansi_Warnings = 1
