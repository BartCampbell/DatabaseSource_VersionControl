...
-- Description:	Initializes entities with a top-ranked criteria based the linked entity's Begin Date.  
--				(Date Comparer: 05AFC2DB-7A80-4270-AF0E-06BA38C21614)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityByEventLinkedBeginDate]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = 'EC15C468-A7D1-4661-B3AF-BD3C4A2F3B19') AND
		...
		SELECT	DATEADD(dd, MEC.AfterDOBDays, DATEADD(mm, MEC.AfterDOBMonths, M.DOB)) AS AfterDOBDate,
				...
				MEC.EntityCritID, 
				DATEADD(dd, MEC.EndDays, DATEADD(mm, MEC.EndMonths, TE.BeginDate)) AS EntityEndDate,
				...
				MEC.RankOrder, TE.BeginDate AS SourceLinkEntityDate, TE.DSEntityID AS SourceLinkID, MEC.ValueMax, MEC.ValueMin
		INTO	#Base
...
GO
GRANT EXECUTE ON  [Batch].[InitializeEntityByEventLinkedBeginDate] TO [Processor]
GO
