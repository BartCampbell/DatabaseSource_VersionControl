...
-- Description:	Peforms the initial data gathering for calculating continuous enrollment for entities based on Begin Date.
--				(Date Comparer: 0C9FEC78-5D04-4AB8-A729-05DD9165C3D2)
-- =============================================
CREATE PROCEDURE [Batch].[InitializeEntityEnrollmentByBeginDateIndexDOBSeed]
(
	@BatchID int,
...
BEGIN
	...
	BEGIN TRY;
		...
		WHERE	(DateComparerGuid = '0C9FEC78-5D04-4AB8-A729-05DD9165C3D2') AND
		...
		SELECT DISTINCT
				...
				DATEADD(dd, EEK.AnchorDays, DATEADD(mm, EEK.AnchorMonths, TEB.BeginDate)) AS AnchorDate,
				DATEADD(dd, EEK.BeginDOBDays, DATEADD(mm, EEK.BeginDOBMonths, TEB.BeginDate)) AS BeginDOBDate, --DOB criteria always on Seed Date
				DATEADD(dd, EEK.BeginDays, DATEADD(mm, EEK.BeginMonths, TEB.BeginDate)) AS BeginEnrollDate,
				DATEADD(dd, EEK.EndDOBDays, DATEADD(mm, EEK.EndDOBMonths, TEB.BeginDate)) AS EndDOBDate, --DOB criteria always on Seed Date
				DATEADD(dd, EEK.EndDays, DATEADD(mm, EEK.EndMonths, TEB.BeginDate)) AS EndEnrollDate,
				EEK.EntityID, EEK.GapDays, EEK.Gaps, EEK.Gender, EEK.MeasEnrollID, EEK.OptionNbr,
				DATEADD(dd, EEK.PayerDays, DATEADD(mm, EEK.PayerMonths, TEB.BeginDate)) AS PayerDate,
				EEK.ProductClassID
		...
		WHERE	(TEB.RankOrder = 1) AND
				(TM.DOB BETWEEN DATEADD(dd, EEK.BeginDOBDays, DATEADD(mm, EEK.BeginDOBMonths, TEB.BeginDate)) AND 
								DATEADD(dd, EEK.EndDOBDays, DATEADD(mm, EEK.EndDOBMonths, TEB.BeginDate))) AND
