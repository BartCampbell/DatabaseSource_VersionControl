		...
		FROM	Measure.EntityCriteria  AS MEC
				...
				INNER JOIN Proxy.Members AS M
						ON TV.DSMemberID = M.DSMemberID AND
							...
							((MEC.DaysMax IS NOT NULL) AND (MEC.DaysMax >= DATEDIFF(dd, TV.BeginDate, ISNULL(TV.EndDate, TV.EndOrigDate)) + 1))) AND
							((MEC.DaysMin IS NULL) OR
							((MEC.DaysMin IS NOT NULL) AND (MEC.DaysMin <= DATEDIFF(dd, TV.BeginDate, ISNULL(TV.EndDate, TV.EndOrigDate)) + 1))) AND
