...
-- Description:	Compiles "EventBase" records to form the actual "Event" records. (v2)
-- =============================================
CREATE PROCEDURE [Batch].[CompileEvents_v2]
(
	@BatchID int
)
AS
BEGIN
	...
	DECLARE @Result int;
		...
		BEGIN TRY;
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#EventMasters ON #EventMasters (EventBaseID) WITH (FILLFACTOR = 100);
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#EventMastersBaseC ON #EventMastersBaseC (DSClaimID, EventID) WITH (FILLFACTOR = 100);
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#EventMastersBaseD ON #EventMastersBaseD (DSMemberID, KeyDate, BeginDate, EndDate, EventID) WITH (FILLFACTOR = 100);
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#EventMastersBaseP ON #EventMastersBaseP (DSMemberID, DSProviderID, KeyDate, BeginDate, EndDate, EventID) WITH (FILLFACTOR = 100);
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#Events ON #Events (EventMasterID, EventID) WITH (FILLFACTOR = 100);
			...
			CREATE UNIQUE CLUSTERED INDEX IX_#KeyEvents ON #KeyEvents (EventMasterID, EventID) WITH (FILLFACTOR = 100);
			CREATE STATISTICS ST_#KeyEvents ON #KeyEvents (EventMasterID, EventID);
						
			SET ANSI_WARNINGS ON;

			--5) Apply any additional date-based requirements for the events and copy them to Proxy.Events--------------------------
			SELECT DISTINCT
					AllowEndDate,
					BeginDate,
					EndDate,
			...
			CREATE UNIQUE CLUSTERED INDEX PK_#DateEventReqs ON #DateEventReqs (EventID) WITH (FILLFACTOR = 100);
			...
			SELECT	V.BatchID, V.BeginDate, V.BeginOrigDate, KV.ClaimTypeID,
					V.CodeID, V.CountClaims, V.CountCodes, 
					V.CountLines, V.CountProviders, 
					@DataRunID, @DataSetID, 
					ISNULL(KV.[Days], V.[Days]) AS [Days], 
					...
					ISNULL(KV.EndOrigDate, V.EndOrigDate) AS EndOrigDate, 
					V.EventCritID, V.EventID, V.EventMasterID AS EventBaseID, V.IsPaid, V.Value
			FROM	#DateEventReqs AS DV
					...
					INNER JOIN dbo.Tally AS T
							ON T.N BETWEEN 1 AND KV.MaxRows AND
