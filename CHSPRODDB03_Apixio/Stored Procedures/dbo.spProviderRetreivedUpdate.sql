...
Description:	Update Date_Retreived for a single or all NULL records in Provider
Usage:		
	EXECUTE dbo.spProviderRetreivedUpdate 
		NULL --  @RecID INT -- NULL For all where Date_Retreived IS NULL
		,NULL -- @Date_Retreived DATETIME -- NULL to let proc set to GETDATE() 
****************************************************************************************************************************************************/
CREATE PROCEDURE [dbo].[spProviderRetreivedUpdate] 
	@RecID INT
