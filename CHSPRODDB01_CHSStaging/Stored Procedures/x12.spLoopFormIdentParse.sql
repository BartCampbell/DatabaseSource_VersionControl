...
BEGIN
	...
	SELECT
		...
		,h.b.value('(LQ/LQ01/text())[1]', 'varchar(50)') AS 'FormIdent_CodeListQualifierCode_LQ01'
		,h.b.value('(LQ/LQ02/text())[1]', 'varchar(50)') AS 'FormIdent_IndustryCode_LQ02'
	FROM 
		x12.shred x
	OUTER APPLY 
	...
	SELECT
		...
		,h.b.value('(FRM01/text())[1]', 'varchar(50)') AS 'FRM_AssignedIdentification_FRM01'
		,h.b.value('(FRM02/text())[1]', 'varchar(50)') AS 'FRM_YesNoConditionOrResponseCode_FRM02'
		,h.b.value('(FRM03/text())[1]', 'varchar(50)') AS 'FRM_ReferenceIdentification_FRM03'
		,h.b.value('(FRM04/text())[1]', 'varchar(50)') AS 'FRM_Date_FRM04'
		,h.b.value('(FRM05/text())[1]', 'varchar(50)') AS 'FRM_PercentDecimalFormat_FRM05'
	FROM 
		x12.shred x
	CROSS APPLY 
