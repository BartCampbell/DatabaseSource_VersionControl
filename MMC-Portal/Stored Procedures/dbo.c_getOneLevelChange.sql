...
-- =============================================
-- c_getOneLevelChange '09/09/2015','10/30/2015',0,0,0,'','',0,1,0
...
,CASE WHEN ISNUMERIC(CPT_EM_Org)=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1))= 1 and ABS(cast(SUBSTRING (CPT_EM_Org,LEN([CPT_EM_Org]),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1) as decimal))=1 THEN 1
			else 0
			END [One Level]
			,CASE WHEN ISNUMERIC(CPT_EM_Org)=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1))= 1 and ABS(cast(SUBSTRING (CPT_EM_Org,LEN([CPT_EM_Org]),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1) as decimal))=2 THEN 1
			else 0
			END [Two Level]
			,CASE WHEN ISNUMERIC(CPT_EM_Org)=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1))= 1 and ABS(cast(SUBSTRING (CPT_EM_Org,LEN([CPT_EM_Org]),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1) as decimal))=3 THEN 1
			else 0
			END [Three Level]
			,CASE WHEN ISNUMERIC(CPT_EM_Org)=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1))= 1 and ABS(cast(SUBSTRING (CPT_EM_Org,LEN([CPT_EM_Org]),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1) as decimal))=4 THEN 1
			else 0
			END [Four Level]
			,CASE WHEN ISNUMERIC(CPT_EM_Org)=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org]),1))= 1 and CHANGE>1 THEN 1
			else 0
			END [category]
...
--Print (@DepartmentQuery)
	...
	BEGIN
		set @FinalQuery='SELECT 
			...
			,CASE
			WHEN ISNUMERIC(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) )=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) )= 1 AND ABS(cast(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) as decimal ))=0 THEN ''0'' 
			WHEN ISNUMERIC(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) )=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) )= 1 AND ABS(cast(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) as decimal ))=1 THEN ''1''
			WHEN ISNUMERIC(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) )=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) )= 1 AND ABS(cast(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) as decimal ))=2 THEN ''2''
			WHEN ISNUMERIC(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) )=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) )= 1 AND ABS(cast(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) as decimal ))=3 THEN ''3''
			WHEN ISNUMERIC(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) )=1 and ISNUMERIC(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) )= 1 AND ABS(cast(SUBSTRING ([CPT_EM_Org],LEN([CPT_EM_Org] ),1) as decimal)-cast(SUBSTRING (CPT_EM_TMI,LEN([CPT_EM_Org] ),1) as decimal ))=4 THEN ''4''
			ELSE ''ERROR''
			END [Level Status]
