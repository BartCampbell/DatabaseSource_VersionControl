...
-- =============================================
CREATE PROCEDURE [dbo].[prExecChaseSummary_Detail] 
...
begin
	...
	from(
		...
		left outer join(
						...
						group by t2.ProviderOffice_PK--,t3.ProjectGroup
						)t4
		--on t2.Project_PK = t4.Project_PK
		on t2.Office_PK = t4.ProviderOffice_PK
		group by t1.User_PK, t1.UserName, t1.Location, t2.LastUpdatedDate--,t4.ProjectGroup
		)t1
	)t1
	group by t1.User_PK, t1.UserName, t1.Location, t1.LastUpdatedDate--,t1.ProjectGroup
...
begin
	...
	from(
		...
		left outer join(
						...
						on t1.Project_PK = t3.Project_PK
						join Adv_112547_001.dbo.tblChannel t4
						on t1.Channel_PK = t4.Channel_PK
						where t3.Project_Name in ('WMR','NMR')
						and t4.Channel_Name = 'Centauri'
						group by  t2.ProviderOffice_PK--, t3.ProjectGroup
						)t4
		--on t2.Project_PK = t4.Project_PK
		on t2.Office_PK = t4.ProviderOffice_PK
...
select	--ClientID,
		...
		--ProjectGroup,
		LastUpdatedDate,
		sum(ContactedOfficeCnt) as ContactedOfficeCnt,
		sum(ContactedChaseCnt) as ContactedChaseCnt,
		sum(ScheduledOfficeCnt) as ScheduledOfficeCnt,
		sum(ScheduledChaseCnt) as ScheduledChaseCnt,
		LoadDate
		--into ChaseDetail
...
group by --ClientID,
		...
		--ScheduledChaseCnt,
		LoadDate
Order by ContactedOfficeCnt desc;
...
--		td.ProjectGroup,
--		td.LastUpdatedDate,
--		td.ContactedOfficeCnt,
--		td.ContactedChaseCnt,
--		td.ScheduledOfficeCnt,
--		td.ScheduledChaseCnt,
--		td.LoadDate
--		from ChaseDetail td
--		Right join ChaseSummary ts
--		on td.Location = ts.Location
--		order by td.UserID
