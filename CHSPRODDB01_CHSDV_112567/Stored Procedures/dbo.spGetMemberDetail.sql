...
AS
    ...
    -- Insert statements for procedure here
        ...
        FROM    [dbo].[H_Member] h
                INNER JOIN dbo.S_MemberDetail s ON s.H_Member_RK = h.H_Member_RK AND s.RecordEndDate IS NULL
        WHERE   ( ISNULL(s.[Latitude], '') <> ''
                  ...
                  OR ISNULL(s.[POA_Name], '') <> ''
				   OR ISNULL(s.[Client_PK], '') <> ''
				  OR ISNULL(s.[Eff_Date], '') <> ''
				  OR ISNULL(s.[Exp_Date], '') <> ''
                )
