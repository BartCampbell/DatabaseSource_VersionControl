...
BEGIN
	SELECT ROW_NUMBER() OVER(
		ORDER BY 
			...
			,IsNull(EQAL.ProviderOfficeInvoice_PK,0) Invoice_PK
		FROM 
			tblProvider P WITH (NOLOCK)
			INNER JOIN tblSuspect S WITH (NOLOCK) ON S.Provider_PK = P.Provider_PK
			INNER JOIN tblProviderMaster PM WITH (NOLOCK) ON PM.ProviderMaster_PK = P.ProviderMaster_PK
			INNER JOIN tblMember M WITH (NOLOCK) ON S.Member_PK = M.Member_PK	
			LEFT JOIN tblExtractionQueueAttachLog EQAL WITH (NOLOCK) ON EQAL.Suspect_PK = S.Suspect_PK AND EQAL.ProviderOfficeInvoice_PK = @invoice
		WHERE P.ProviderOffice_PK=@Office
		ORDER BY Invoice_PK DESC,Member
END
GO
