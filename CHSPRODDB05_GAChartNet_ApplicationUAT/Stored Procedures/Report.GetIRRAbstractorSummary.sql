...
CREATE PROCEDURE [Report].[GetIRRAbstractorSummary]
(
...
BEGIN
	...
	(
		...
		FROM	dbo.AbstractionReview AS AR WITH(NOLOCK)
				...
				LEFT OUTER JOIN Deductions AS D WITH(NOLOCK)
	...
	(
		SELECT	R.AbstractorID,
				COUNT(DISTINCT RV.PursuitEventID) AS CountRecords,
	...
	SELECT	t.AbstractorID,
            MIN(t.AbstractorName) AS AbstractorName,
			CONVERT(int, @ReviewerID) AS ReviewerID,
			CONVERT(int, @AbstractionReviewStatusID) AS AbstractionReviewStatusID,
	...
	SELECT	t.AbstractorID,
			...
			2 AS SortOrder
	FROM	Results AS t
	ORDER BY SortOrder, MeasureAbbrev, MeasureComponentName, AbstractorName
	OPTION	(OPTIMIZE FOR UNKNOWN);
		
END

GO
GRANT EXECUTE ON  [Report].[GetIRRAbstractorSummary] TO [Reporting]
GO
