...
-----------Transaction Starts-------------------
			...
			BEGIN TRY
				...
				SELECT Project_PK,@office,@dt_from,@dt_to,@ScanTech,@Usr,GetDate(),0,0 FROM cacheProviderOffice WHERE ProviderOffice_PK=@office
				COMMIT TRANSACTION
			END TRY
			BEGIN CATCH
...
-----------Transaction Starts-------------------
			...
			BEGIN TRY
				...
				SELECT Project_PK,@office,DateAdd(day,@followup,GetDate()),DateAdd(day,@followup,GetDate()),@ScanTech,@Usr,GetDate(),@schedule_type,@followup,@AddInfo FROM cacheProviderOffice WHERE ProviderOffice_PK=@office
				COMMIT TRANSACTION
			END TRY
			BEGIN CATCH
				ROLLBACK TRANSACTION
...
-----------Transaction Starts-------------------
			...
			BEGIN TRY
				...
				SELECT Project_PK,@office,2,@ScheduleInfo,@Usr,GetDate(),@contact_num FROM cacheProviderOffice WHERE ProviderOffice_PK=@office
				COMMIT TRANSACTION
			END TRY
			BEGIN CATCH
				ROLLBACK TRANSACTION
