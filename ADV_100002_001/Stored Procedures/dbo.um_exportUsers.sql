...
BEGIN
	SELECT CASE WHEN IsActive=1 THEN 'YES' END [Active]
		...
		,CASE WHEN IsCoderOnsite=1 THEN 'YES' END  [Coder On-site]
		,CASE WHEN IsCoderOffsite=1 THEN 'YES' END  [Coder Off-site]
		,CASE WHEN IsCodingManager=1 THEN 'YES' END  [Coding Manager]
		,CASE WHEN IsQAManager=1 THEN 'YES' END  [QA Manager]
		,CASE WHEN IsQACoder=1 THEN 'YES' END  [QA Coder]
