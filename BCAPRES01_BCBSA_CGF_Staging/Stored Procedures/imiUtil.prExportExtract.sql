...
create PROC [imiUtil].[prExportExtract] 
...
BEGIN
	...
	EXEC IMIUtil.spExportDataToXLS
		@dbName			= @vcDB,
		@sql			= @vcCmd,
		@fullFileName	= @vcCmd2
...
BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					SELECT @vcCmd = @vcCmd + COLUMN_NAME + '|'
						FROM #col
						WHERE ORDINAL_POSITION < (SELECT MAX(ORDINAL_POSITION) FROM #col)
						ORDER BY ORDINAL_POSITION
	...
	BEGIN
		...
		SET @vcCmd3 = 'bcp ' 
			+ '"select ExtrTxt from ##ExtrOut order by ExtrRowid" '
			+ '  QUERYOUT '
			+' "'+ @vcTargetPath + @vcTargetFileName
			+ '" -S ' + @@ServerName
			+ ' -T -c ' 
			+ ' -o ' + RTRIM(@vcTargetPath)+'prExportExtract.out '
			+ ' -e ' + RTRIM(@vcTargetPath)+'prExportExtract.err '
	
		IF @bDebug = 1
			PRINT @vcCmd3

		EXEC master..xp_cmdshell @vcCmd3
	...
	IF @bZipFile = 1
	BEGIN
		SET @vcCmd2 = 'c:\ihds\7za a ' + @vcTargetPath + LEFT(@vcTargetFileName,LEN(@vcTargetFileName)-4) 
					+ '.zip ' + @vcTargetPath + @vcTargetFileName

		IF @bDebug = 1
			PRINT @vcCmd2
		EXEC xp_cmdshell @vcCmd2
		
		SET @vcCmd2 = 'del ' + @vcTargetPath + @vcTargetFileName

		IF @bDebug = 1
			PRINT @vcCmd2
		EXEC xp_cmdshell @vcCmd2
		
		SET @vcCmd2 = 'del ' + @vcTargetPath + 'prExportExtract.err'

		IF @bDebug = 1
			PRINT @vcCmd2
		EXEC xp_cmdshell @vcCmd2

		SET @vcCmd2 = 'del ' + @vcTargetPath + 'prExportExtract.out'
