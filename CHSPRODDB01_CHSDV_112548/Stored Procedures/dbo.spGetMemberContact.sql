    ...
    -- Insert statements for procedure here
        ...
        SELECT  h.Member_BK AS CentauriMemberID ,
          ...
          FROM    [dbo].[H_Member] h
				...
				LEFT OUTER JOIN [dbo].[LS_MemberContactType] t ON t.[L_MemberContact_RK] = L.[L_MemberContact_RK] AND t.RecordEndDate IS NULL
				WHERE (ISNULL(s.Phone ,'') <>''  OR ISNULL(s.Cell,'')<>'' OR ISNULL(s.EmailAddress,'')<>'')
				AND  (h.LoadDate> @LoadDate	OR s.LoadDate> @LoadDate OR t.LoadDate>@LoadDate			)
				;
