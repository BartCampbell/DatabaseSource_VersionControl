...
CREATE PROCEDURE [Report].[GetIRRDetail]
(
...
BEGIN
	...
	(
		SELECT	R.AbstractionDate,
				...
				ARD.CriticalVsAll,
				ARD.CriticalVsAllPercentage,
				ARD.FixedVsAll,
				ARD.FixedVsAllPercentage,
				ARD.CountCriticalFixed,
                ARD.CountCriticalNotFixed,
                ARD.CountNotCriticalFixed,
                ARD.CountNotCriticalNotFixed,
				ISNULL(ARD.IsFixed, 0) AS IsFixed,
				ISNULL(ARD.IsCritical, 0) AS IsCritical,
				ARST.IsCanceled,
				ARST.IsCompleted, 
		...
		FROM	dbo.AbstractionReview AS AR WITH(NOLOCK)
				...
				OUTER APPLY (
								SELECT	MIN(CASE WHEN tARD.IsFixed = 1 THEN 1 ELSE 0 END) AS IsFixed, 
										MAX(CASE WHEN tARD.IsCritical = 1 THEN 1 ELSE 0 END) AS IsCritical, 
										CONVERT(varchar(32), COUNT(DISTINCT CASE WHEN tARD.IsCritical = 1 THEN tARD.AbstractionReviewDetailID END)) + ' of ' +
											CONVERT(varchar(32), COUNT(DISTINCT tARD.AbstractionReviewDetailID)) AS CriticalVsAll, 
										COUNT(DISTINCT CASE WHEN tARD.IsCritical = 1 THEN tARD.AbstractionReviewDetailID END) /
											NULLIF(COUNT(DISTINCT tARD.AbstractionReviewDetailID), 0) AS CriticalVsAllPercentage, 
										CONVERT(varchar(32), COUNT(DISTINCT CASE WHEN tARD.IsFixed = 1 THEN tARD.AbstractionReviewDetailID END)) + ' of ' +
											CONVERT(varchar(32), COUNT(DISTINCT tARD.AbstractionReviewDetailID)) AS FixedVsAll, 
										COUNT(DISTINCT CASE WHEN tARD.IsFixed = 1 THEN tARD.AbstractionReviewDetailID END) /
											NULLIF(COUNT(DISTINCT tARD.AbstractionReviewDetailID), 0) AS FixedVsAllPercentage, 
										SUM(CASE WHEN tARD.IsCritical = 1 AND tARD.IsFixed = 1 THEN 1 ELSE 0 END) AS CountCriticalFixed, 
										SUM(CASE WHEN tARD.IsCritical = 1 AND tARD.IsFixed = 0 THEN 1 ELSE 0 END) AS CountCriticalNotFixed, 
										SUM(CASE WHEN tARD.IsCritical = 0 AND tARD.IsFixed = 1 THEN 1 ELSE 0 END) AS CountNotCriticalFixed, 
										SUM(CASE WHEN tARD.IsCritical = 0 AND tARD.IsFixed = 0 THEN 1 ELSE 0 END) AS CountNotCriticalNotFixed 
	...
	SELECT	ISNULL(A.AbstractorID, -1) AS AbstractorID,
			...
			MBR.DateOfBirth,
			MBR.Gender,
			ISNULL(V.CriticalVsAll, '0 of 0') AS CriticalVsAll,
			V.CriticalVsAllPercentage,
			ISNULL(V.FixedVsAll, '0 of 0') AS FixedVsAll,
			V.FixedVsAllPercentage,
			--ISNULL(CASE WHEN V.IsCompleted = 1 THEN V.AbstractionReviewID END, 0) AS CountReviewCompleted,
			--ISNULL(CASE WHEN V.IsCompleted = 0 THEN V.AbstractionReviewID END, 0) AS CountReviewNotCompleted,
			--ISNULL(V.CountCriticalFixed, 0) AS CountReviewDetailCriticalFixed,
		    --ISNULL(V.CountCriticalNotFixed, 0) AS CountReviewDetailCriticalNotFixed,
		    --ISNULL(V.CountNotCriticalFixed, 0) AS CountReviewDetailNotCriticalFixed,
		    --ISNULL(V.CountNotCriticalNotFixed, 0) AS CountReviewDetailNotCriticalNotFixed,
			--ISNULL(V.IsCritical, 0) AS IsCritical,
...
END
GO
GRANT EXECUTE ON  [Report].[GetIRRDetail] TO [Reporting]
GO
