...
-- =============================================
CREATE PROC [dbo].[spAdvMergeMemberDetailWC]
AS
    ...
    BEGIN
                ...
                FROM    stage.MemberDetail_ADV s
                        ...
                        LEFT JOIN dim.MemberDetail d ON d.MemberID = m.MemberID
                                                        ...
                                                        AND ISNULL(d.[POA_Name],
                                                              '') = ISNULL(s.[POA_Name],
                                                              '')
                                                        AND ISNULL(d.[Client_PK],
                                                              '') = ISNULL(s.[Client_PK],
                                                              '')
                                                        AND ISNULL(d.[Eff_Date],
                                                              '') = ISNULL(s.[Eff_Date],
                                                              '')
                                                        AND ISNULL(d.[Exp_Date],
                                                              '') = ISNULL(s.[Exp_Date],
                                                              '')
        ...
        WHERE   ( ISNULL(mc.[Latitude], 0.00) <> ISNULL(s.[Latitude], 0.00)
                  ...
                  OR ISNULL(mc.[Client_PK], '') <> ISNULL(s.[Client_PK], '')
                  OR ISNULL(mc.[Eff_Date], '') <> ISNULL(s.[Eff_Date], '')
                  OR ISNULL(mc.[Exp_Date], '') <> ISNULL(s.[Exp_Date], '')
                )
