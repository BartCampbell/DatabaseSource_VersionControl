...
Notes:		
Process:	
Test Script:	exec [CGF_REP].[rptMemberInfo] 
					@ServiceDate = '20141231',
					@IHDS_Member_ID = 200485
...
--/*
CREATE PROC [CGF_REP].[rptMemberInfo] 
...
DECLARE @vcMMStart VARCHAR(8) = CONVERT(VARCHAR(8),DATEADD(MOnth,(-1*(@Mthsback-1)),@ServiceDate),112)
...
SELECT  ParamServicedate = @ServiceDate ,
		...
		MbrInfo.State,
		MbrInfo.ZipCode,
		CityStateZip = CASE WHEN MbrInfo.City IS NULL	
							THEN CASE WHEN mbrinfo.State IS NULL 
										THEN MbrInfo.ZipCode
									ELSE RTRIM(mbrinfo.State) + ', ' + ISNULL(MbrInfo.ZipCode,'')
									END
							ELSE RTRIM(MbrInfo.City) + ', ' + RTRIM(ISNULL(MbrInfo.State,'')) + ' ' + ISNULL(MbrInfo.ZipCode,'')
							END,
		MbrInfo.PhoneHome,
		MbrInfo.PhoneMobile,
		MbrInfo.PhoneWork,
		...
		mbrinfo.CustomerMemberID,
		mbrinfo.CustomerDiamondID,
		...
		LastServicedate = MedClm.LastServiceDate
	...
	FROM (
			...
			FROM Member m
				LEFT JOIN (SELECT mmf.MemberID,
							...
							WHERE mmf.MemberID= @MemberID
								AND MonthDate BETWEEN @vcMMStart AND @vcMMEnd
		...
		LEFT JOIN (SELECT c.MemberID,
						...
						LastServiceDate = MAX(cli.DateServiceBegin), 
					...
					WHERE c.MemberID= @MemberID
						AND CONVERT(VARCHAR(8),cli.DateServiceBegin,112) BETWEEN @vcServiceBegin AND @vcServiceEnd
					GROUP BY c.MemberID
				) MedClm
		...
		LEFT JOIN (SELECT pc.MemberID,
					...
					WHERE pc.MemberID = @MemberID
						AND CONVERT(VARCHAR(8),DateDispensed,112) BETWEEN @vcServiceBegin AND @vcServiceEnd
...
*/
GO
GRANT VIEW DEFINITION ON  [CGF_REP].[rptMemberInfo] TO [db_ViewProcedures]
GO
