...
BEGIN
	...
	SELECT ROW_NUMBER() OVER(
		ORDER BY 
			...
			CASE WHEN @Order='ASC'  THEN CASE @SORT WHEN 'AC' THEN IsActive WHEN 'C' THEN IsCoder WHEN 'A' THEN IsAdmin WHEN 'CS' THEN isCoderSupervisor WHEN 'CM' THEN IsClient ELSE NULL END END ASC,
			CASE WHEN @Order='DESC' THEN CASE @SORT WHEN 'AC' THEN IsActive WHEN 'C' THEN IsCoder WHEN 'A' THEN IsAdmin WHEN 'CS' THEN isCoderSupervisor WHEN 'CM' THEN IsClient ELSE NULL END END DESC
		) AS RowNumber
			,User_PK,IsActive,Username,Lastname+', '+Firstname Fullname,Email_Address,IsAdmin,IsCoder,isCoderSupervisor,IsClient
