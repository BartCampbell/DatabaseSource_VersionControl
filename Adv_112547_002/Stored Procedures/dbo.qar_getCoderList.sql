...
BEGIN
	...
	SELECT ROW_NUMBER() OVER(ORDER BY U.Lastname) RowNumber,
		...
		--100 - [(Changed + Added + Removed)/(Confirmed + Added)
		CASE WHEN COUNT(DISTINCT CASE WHEN CDQ.IsConfirmed=1 THEN CDQ.CodedData_PK ELSE NULL END)+COUNT(DISTINCT CASE WHEN CDQ.IsAdded=1 THEN CDQ.CodedData_PK ELSE NULL END)=0 THEN '' ELSE
			CAST(100 - ROUND(

				CAST(COUNT(DISTINCT CASE WHEN CDQ.IsChanged=1 THEN CDQ.CodedData_PK ELSE NULL END)+COUNT(DISTINCT CASE WHEN CDQ.IsAdded=1 THEN CDQ.CodedData_PK ELSE NULL END)+COUNT(DISTINCT CASE WHEN CDQ.IsRemoved=1 THEN CDQ.CodedData_PK ELSE NULL END) AS Float)
				/
				CAST(COUNT(DISTINCT CASE WHEN CDQ.IsConfirmed=1 THEN CDQ.CodedData_PK ELSE NULL END)+COUNT(DISTINCT CASE WHEN CDQ.IsAdded=1 THEN CDQ.CodedData_PK ELSE NULL END) AS Float)*100
				
			,2) AS varchar) + ' %' END [QA Accuracy Rate]
